...
deux formes (tu es déjà dans le bon répertoire ; toute variante — chemin \
absolu, cd, PowerShell — sera bloquée) :
  {python} {requete} "SELECT …"
  {python} {requete} outil <nom> '{{"query": "…"}}'
  (le JSON entre quotes SIMPLES — exemple complet :
  {python} {requete} outil peut_detruire '{{"query": "avenger titan", \
...
def repondre(question: str, *, fournisseur: str | None = None,
    ...
    try:
        return _interroger(question, cle, fournisseur, modele, session)
    finally:
        with _EN_VOL_VERROU:
            _EN_VOL.pop(cle).set()
...
def _consignes(acces: str) -> str:
    ...
    requete = "scripts/requete.py"
    return _CONSIGNES.format(
        acces=acces.format(python=python, requete=requete),
...
def _interroger_claude(cli: list[str], question: str,
    ...
    return subprocess.run(
        [*cli, "-p", question,
         ...
         "--allowedTools", "Bash(.venv/Scripts/python.exe "
         "scripts/requete.py:*)",
         "--max-turns", "12"],
...
def _commande_codex_exec(cli: list[str], question: str,
    ...
    for cle, valeur in (
        ...
        ("mcp_servers.disco_analyste.command", str(python)),
        ("mcp_servers.disco_analyste.args", [str(serveur)]),
        ("mcp_servers.disco_analyste.cwd", str(config.PROJECT_ROOT)),
