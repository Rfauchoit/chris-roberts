...
_PROMESSES = {
    ...
    "parcours": "le guide complet du démarrage — chaînes dans l'ordre, "
    ...
    # Le terminal règle ce que la réponse ne peut que décrire : un curseur
    # par emplacement, et le compte de balles qui suit le montage.
    "fabricateur": "les curseurs de qualité, emplacement par emplacement, "
                   "et le compte de balles du montage",
}
...
def _cible(outil: str, data: dict[str, Any]) -> tuple[str | None, dict]:
    ...
    if outil == "par_ou_commencer":
        # Le guide de démarrage a sa page depuis le 2026-08-19 : la règle,
        # les chaînes dans l'ordre, prérequis et récompenses — plus riche
        # que l'onglet trié où ce lien menait avant.
        return "parcours", {}
    ...
    # --- Le terminal de fabrication : la réponse dit **où** mettre le bon
    # matériau, l'écran laisse le régler et lire le résultat. Sans schéma
    # identifié on ne lie pas : ouvrir la recherche ferait retaper le nom
    # qu'on vient de lire.
    if outil == "fabricateur":
        uuid = data.get("blueprint_uuid")
        return ("fabricateur", {"bp": uuid}) if uuid else (None, {})
