...
# Combien de temps on accepte d'attendre **rien du tout**. Mesuré le
# 2026-08-16 : sur Windows, un refus de connexion en loopback prend
# 2 045 ms — le refus lui-même est lent, indépendamment du timeout HTTP.
# La cascade appelant l'étage local deux fois, une machine sans
# llama-server payait 4,1 s par question non routée. Une sonde à 150 ms
# suffit largement pour un serveur qui tourne sur la même machine.
LOCAL_LLM_SONDE = float(os.environ.get("DISCO_LOCAL_LLM_SONDE", "0.15"))
...
# 16384 depuis le 2026-08-14 : le 77e outil (`par_ou_commencer`) a porté
# l'estimation à 10 500 contre 10 444 tolérés. Deuxième fois que la garde
# sonne, deuxième fois qu'elle a raison — et elle sonne alors même que
# l'étage local est éteint en production, ce qui est le but : le jour où on
# le rallume, le catalogue doit rentrer.
LOCAL_LLM_CONTEXT = int(os.environ.get("DISCO_LOCAL_LLM_CONTEXT", "16384"))
...
# **L'adresse du cockpit, telle qu'un lecteur peut la taper.** Chris renvoie
# vers ses pages — « la liste complète est sur le cockpit » —, et le lien doit
# être atteignable par celui qui lit, pas par la machine qui répond.
#
# Le fichier, plutôt qu'une variable d'environnement : le tunnel s'ouvre
# **pendant** que le cœur tourne déjà, et un `os.environ` posé après le
# démarrage d'un processus ne lui parvient jamais. Le lanceur écrit l'adresse
# publique ici quand il publie le serveur, l'efface quand il ferme, et le
# cœur la relit à chaque réponse — c'est un fichier d'une ligne, pas une
# requête.
#
# Ordre : la surcharge explicite, puis le fichier, puis `CORE_URL`. Le
# dernier ne vaut que sur la machine de l'hôte, et c'est assumé : sans
# tunnel ouvert, personne d'autre ne peut joindre le cockpit de toute façon.
COCKPIT_URL = os.environ.get("DISCO_COCKPIT_URL", "")
COCKPIT_URL_FICHIER = pathlib.Path(os.environ.get(
    "DISCO_COCKPIT_URL_FICHIER", DATA_DIR / "cockpit_url.txt"))
