...
def _phrase_pledge(nom: str, msrp) -> str:
    """« L'Aurora ne se vend pas en aUEC » est une réalité du jeu, pas un trou.

    Journal du 2026-08-19 : « combien coûte un aurora » → « je n'ai pas de
    prix ». Le vaisseau n'a aucun relevé UEX parce qu'il ne se vend pas en
    jeu pour l'instant ; son prix existe — en dollars, au pledge store,
    relevé par le Star Citizen Wiki. On nomme la source : ce n'est ni un
    relevé UEX ni un fichier du jeu.
    """
    dollars = f"{msrp:g}".replace(".", ",")
    return (f"{nom} ne se vend pas en aUEC d'après les relevés — il "
            f"s'obtient au pledge store, en argent réel : **{dollars} $**. "
            "_Prix relevé par le Star Citizen Wiki._")
...
def render_price(data: dict[str, Any]) -> str:
    ...
    texte += _note_de_prix_observe(data)
    for autre in data.get("autres_prix") or []:
        texte += "\n\n" + render_price(autre)
    return texte


def _note_de_prix_observe(data: dict[str, Any]) -> str:
    """« Tu l'as payé 375 le 10 août » — l'observation, dite comme telle.

    **Ce n'est pas une donnée de jeu**, et la phrase doit l'annoncer : le
    §7 garantit que les chiffres viennent de la base, celui-ci vient du
    Game.log d'un membre. La confusion serait grave — un joueur qui lit un
    prix croit à une cotation.

    Trois choix mesurés :

    - **on donne la date**, parce qu'une observation unique n'est pas une
      cotation. « 375 aUEC » sans date se lirait comme un prix courant ;
    - **le lieu n'apparaît que si on le connaît** — 11 boutiques sur 27
      seulement portent un nom rapprochable du starmap, et promettre « à
      tel terminal » sur 41 % des cas serait une demi-vérité ;
    - **l'écart n'est souligné que s'il est net.** À 5 % près, deux
      relevés qui divergent ne disent rien ; à 20 %, la cotation UEX a
      probablement vieilli, et c'est **elle** que le joueur doit croire le
      moins.
    """
    observe = data.get("observe")
    if observe is None:
        return ""
    dernier = observe.dernier
    quand = (dernier.quand or "")[:10]
    combien = len(observe.achats)
    phrase = (f"\n\n_Observé : payé **{_nombre(dernier.prix)} "
              f"{dernier.devise}** le {quand}")
    if combien > 1:
        phrase += f", sur {_plural(combien, 'achat')} relevés"
    phrase += " dans nos parties"
    ecart = observe.ecart
    if ecart is not None and observe.prix_uex and (
            abs(ecart) / observe.prix_uex >= 0.20):
        sens = "au-dessus" if ecart > 0 else "en dessous"
        phrase += (f" — soit {_nombre(abs(ecart))} {sens} du meilleur prix "
                   f"listé sur UEX, "
                   f"qui a peut-être vieilli")
    return phrase + "._"
...
def _render_price_un(data: dict[str, Any]) -> str:
    ...
    if portee == "achat":
        ...
        if data.get("msrp"):
            return _phrase_pledge(nom, data["msrp"])
