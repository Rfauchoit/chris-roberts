    ...
    **Trois questions, un calcul, trois attaques différentes.** Mener par « il
    ...
    if len(data["composants"]) > 1:
        lignes.append(f"_Hypothèse : les {len(data['composants'])} composants "
                      ...
                      f"sont fabriqués à la même qualité. Le jeu publie leur "
                      f"effet séparément et ne dit pas comment ils se combinent._")
...
def render_classement_equipement(data) -> str:
    ...
    sens = "les plus basses" if data.moins_est_mieux else "les plus élevées"
    ...
    lignes = [f"Les {data.famille}s{rarete}{budget}{systeme} avec les {data.libelle} "
              f"{sens} — sur {_nombre(data.total)} mesurées :", ""]
    ...
    *ce qu'il veut pouvoir faire* ; la qualité en découle. Chaque ligne
    ...
    if carte:
        ...
        for cible in sorted(possibles, key=lambda c: c["qualite"]):
            lignes.append(
                f"- {_cible_lisible(cible)} — dès la qualité "
                f"**{int(cible['qualite'])}**")
        ...
        for cible in jamais:
            manque = cible.get("manque_pct")
            chiffre = (f" — il manque {str(manque).replace('.', ',')} % "
                       "des points de vie" if manque is not None else "")
            lignes.append(f"- {_cible_lisible(cible)} : jamais{chiffre}")
