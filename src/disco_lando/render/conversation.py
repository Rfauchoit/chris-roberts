...
def render_repli(data: dict[str, Any]) -> str:
    """« Je n'ai pas compris » devient « voilà ce que je sais en dire ».

    Demande de l'utilisateur (2026-08-16) : ne plus se taire dès lors qu'on
    nous parle. La question n'a pas été routée, mais elle a **nommé quelque
    chose** — et ce quelque chose, on sait souvent en dire beaucoup.

    Le cas de la comparaison est le plus intéressant : « y a-t-il mieux que
    le Gladius » n'a pas de réponse tant que « mieux » n'a pas de critère.
    Ce n'est pas une raison de se taire — c'est une raison de **demander
    lequel**, en nommant les axes qu'on sait classer.
    """
    nom = data["entite"]
    axes = data["axes"]
    lignes: list[str] = []
    if data.get("comparaison"):
        # Les axes s'opposent réellement — le meilleur en dégâts est rarement
        # le meilleur en soute. Le dire évite que « je ne choisis pas » passe
        # pour une dérobade.
        lignes.append(
            f"« Mieux » sur quoi ? Le meilleur sur un axe est rarement le "
            f"meilleur sur les autres, et je ne choisis pas le critère à ta "
            f"place. Face à **{nom}**, je sais classer sur :")
    elif data.get("comparaison_impossible"):
        # La question était limpide et c'est **nous** qui ne savons pas
        # classer cette famille — le dire vaut mieux que renvoyer au joueur
        # un défaut de compréhension qu'il n'a pas commis.
        lignes.append(f"Je ne sais pas encore classer cette famille — il me "
                      f"manque de quoi comparer. Sur **{nom}**, je peux te "
                      f"dire :")
    else:
        lignes.append(f"Je n'ai pas saisi ce que tu veux savoir sur "
                      f"**{nom}**. Ce que je sais en dire :")
    lignes.append("")
    lignes += [f"- {axe['libelle']} — « {axe['question']} »" for axe in axes]
    return "\n".join(lignes)


_RENDERERS["repli"] = render_repli
...
def sujet(tool: str, data: dict[str, Any], defaut: str | None) -> str | None:
    ...
    if tool == "catalogue_objets":
        ...
        for groupe in (data.get("groupes") or {}).values():
            for entree in groupe.values():
                return entree["nom"]
    if tool in ("qualite_pour_tuer", "fabricateur"):
        ...
        # « et dans les jambes ? » tombait dans le vide. Le poste de
        # fabrication résout par le même chemin et tombait dans le même trou.
    ...
    if tool in ("activites_a_faire", "par_ou_commencer"):
        # Un outil sans entité doit déclarer son sujet, sinon le « oui »
        # suivant tombe dans le vide. Celui d'une liste d'activités est sa
        # première — « raconte-moi » et « le détail » enchaînent dessus.
        premier = (data.get("activites") or [{}])[0].get("nom")
        if premier:
            return premier
    return defaut
...
def suites(tool: str, data: dict[str, Any], *,
    ...
    if tool == "get_price":
    ...
    elif tool == "fabricateur":
        # **Ce que le poste annonce doit être exécutable.** Le rendu propose
        # la simulation à 900 puis les gisements ; sans ces deux suites, un
        # « oui » ne trouverait rien à reprendre — le défaut du volet
        # « missions », payé deux fois au journal.
        nom = data.get("nom")
        postes = [p for palier in data.get("paliers") or []
                  for p in palier.get("postes") or []]
        if nom and not postes:
            # Sans emplacement, le rendu renvoie vers la recette : elle doit
            # être reprenable, sinon le « oui » tombe dans le vide.
            proposees.append(Suite(
                "la recette complète", "get_blueprint",
                {"query": nom, "volet": "recette"}, exhaustif=True,
                defaut=True,
                mots=("recette", "fabrication", "fabriquer", "craft",
                      "ingredient", "ingredients", "materiaux", "materiau")))
            return proposees
        if nom and data.get("qualite") is None:
            proposees.append(Suite(
                "ce que ça donne avec des matériaux de qualité 900",
                "fabricateur", {"query": nom, "qualite": 900.0},
                defaut=True,
                mots=("qualite", "900", "990", "materiaux", "materiau",
                      "donne", "change")))
        if nom:
            proposees.append(Suite(
                "où miner ces matériaux", "ou_miner_pour", {"query": nom},
                exhaustif=True,
                # Le défaut ne se devine pas : quand la qualité est déjà
                # posée, c'est la seule proposition qui reste.
                defaut=data.get("qualite") is not None,
                mots=("miner", "minage", "gisement", "gisements", "trouver",
                      "materiaux", "materiau", "minerai", "minerais", "ou")))
        ...
        if isinstance(groupes, dict) and any(
            ...
            # `seuil_ignore` voyage avec les filtres : sans lui, « la liste
            # entière » après « plus de 5 000 de capacité » reperdait la
            # mention du critère non honoré, et le joueur croyait sa
            # contrainte appliquée. Une contrainte se dit à chaque tour où
            # la réponse en dépend, pas seulement au premier.
    ...
    elif tool in ("activites_a_faire", "par_ou_commencer"):
        # « Oui » après une liste d'activités ne trouvait rien à reprendre.
        # La suite naturelle est le **détail** de l'une d'elles : c'est la
        # seule chose de plus qu'on sache dire sur une ligne de la liste. On
        # en propose trois — « on annonce peu et on accepte large ».
        for rang, fiche in enumerate((data.get("activites") or [])[:3]):
            proposees.append(Suite(
                f"le détail de « {fiche['nom']} »", "fiche_activite",
                {"query": fiche["cle"]}, entite_proposee=True,
                defaut=(rang == 0),
                mots=("detail", "details", "fiche", "deroule", "etapes",
                      "materiel", "comment", "raconte")
                + tuple(_normalize(fiche["nom"]).split())))
...
def render(tool: str, data: dict[str, Any], *, exhaustif: bool = False) -> str:
    ...
    if exhaustif and tool in _EXHAUSTIFS:
        texte = _EXHAUSTIFS[tool](data)
    else:
        texte = renderer(data)
    return _avec_lien_cockpit(tool, data, texte)


def _avec_lien_cockpit(tool: str, data: dict[str, Any], texte: str) -> str:
    """La ligne « et sur le cockpit… », quand la page va plus loin.

    **Ici et pas dans chaque rendu.** Trente rendus qui collent chacun leur
    lien finiraient par diverger — un oubli au prochain outil, une tournure
    différente au suivant — et la règle « on ne lie que ce que la page fait
    mieux » ne serait plus vérifiable d'un seul endroit. La table vit dans
    `cockpit.py`, la phrase ici, et `render()` est le point de passage
    unique par lequel tous les frontends sortent.

    Demande de l'utilisateur, 2026-08-14 : « fais en sorte que ces questions
    du bot le redirigent vers la page du cockpit ».
    """
    if not isinstance(data, dict):
        return texte
    try:
        from .. import cockpit
        trouve = cockpit.lien(tool, data)
    except Exception:                                          # noqa: BLE001
        # Un lien est un confort ; il ne fait jamais tomber une réponse de
        # jeu. C'est le contrat des étages qui s'ajoutent au déterministe.
        return texte
    if trouve is None:
        return texte
    adresse, promesse = trouve
    return f"{texte}\n\n{cockpit.PREFIXE} — {promesse} : {adresse}*"
