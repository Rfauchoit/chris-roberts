...
def dit_stat(stat: str, valeur: Any) -> str:
    ...
    if stat == "reload_seconds":
        # « a rechargement : 1 secondes » sortait du repli générique. Le
        # français met la seconde au singulier sous 2 — « 1,5 seconde ».
        return (f"besoin de {_nombre(valeur)} "
                f"{'seconde' if valeur < 2 else 'secondes'} pour recharger")
