...
from ..armurerie import rayon_fr
from .socle import (
    _RENDERERS,
    _mult,
    _nom_stat,
...
def render_item_stats(data: dict[str, Any]) -> str:
    ...
    # **Le rayon de boutique ne parle que quand rien d'autre ne parle.**
    # `weapon_class` couvre les armes et laisse muettes les fiches
    # d'armure, d'accessoire et de composant : leur en-tête se réduisait au
    # nom et à la taille, sans jamais dire ce que l'objet **est**. Le rayon
    # le dit — « casque », « optique », « sac à dos ». Il reste en repli
    # pour ne pas doubler la classe d'arme, qui est plus fine.
    if not qualif and (categorie := rayon_fr(item.get("rayon"))):
        qualif.append(categorie)
...
def render_objets_seuil(data: dict[str, Any]) -> str:
    ...
    # Le libellé vient de deux sources : `_weapon_filter` l'écrit en anglais
    # (« ballistic cannon ») et il faut inverser l'ordre des mots ;
    # `detect_famille_objets` l'écrit déjà en français, et la même inversion
    # rendait « FPS armes » et « vaisseau de armes ». La requête dit laquelle.
    famille = (data["famille"] if data.get("famille_deja_fr")
               else _famille_fr(data["famille"], data["famille"])) \
        if data.get("famille") else ""
    # **Le libellé porte parfois déjà le nom.** « armes FPS » ajouté au
    # décompte donnait « 222 armes armes FPS » ; quand la famille commence
    # par le nom, c'est elle qu'on décline, pas un « arme » de plus.
    tete = (f"{data['total']} {famille}" if famille.startswith("arme")
            else f"{_plural(data['total'], 'arme')}"
                 + (f" {famille}" if famille else ""))
    lignes = [f"**{tete}** font {sens} "
              f"**{_nombre(data['valeur'])}{unite}** de {data['libelle']} :", ""]
    # **Un DPS ne vaut que le temps qu'on peut le tenir.** Le Boomtube
    # publie 341 833 dégâts par seconde et porte **une** roquette : 0,12
    # seconde de tir. Sans cette durée, le classement met en tête une arme
    # qui ne soutient son chiffre sur aucune seconde entière — c'est le
    # piège du Destroyer Mass Driver, appliqué au temps plutôt qu'à la
    # taille. Remarque de l'utilisateur, 2026-08-21.
    #
    # Elle ne s'affiche que sous cinq secondes : au-delà, le débit décrit
    # bien l'arme et la précision devient du bruit.
    lignes += [f"- {i['name']} — {_nombre(i['valeur'])}{unite}"
               + (f", taille {i['size']}" if i.get("size") is not None else "")
               + (f" — **{_nombre(i['dps_avec_recharge'])}{unite}** "
                  f"chargeur compris ({_nombre(i['secondes_de_tir'])} s de "
                  f"tir, puis on recharge)"
                  if i.get("dps_avec_recharge")
                  and i["dps_avec_recharge"] < i["valeur"] * 0.9 else "")
               for i in data["items"]]
...
def render_catalogue_objets(data: dict[str, Any], montrer: int = 10) -> str:
    ...
    for groupe in sorted(data["groupes"],
        ...
        # `.capitalize()` force le reste en minuscules et abîmait les noms
        # propres depuis que les catalogues se groupent aussi par marque :
        # « Behring Applied Technology » ressortait « Behring applied
        # technology ». On ne touche qu'à la première lettre.
        lignes.append(f"- *{groupe[:1].upper() + groupe[1:]}* : "
                      + ", ".join(noms) + suite)
    # Une contrainte comprise mais non appliquée se dit. « Plus de 5 000 de
    # capacité » sur des boucliers rendait le catalogue entier avec l'air
    # d'avoir répondu — la réponse partielle qui se fait passer pour
    # complète est le pire des cas.
    if seuil := data.get("seuil_ignore"):
        sens = "plus de" if seuil[0] == ">=" else "moins de"
        lignes += ["", f"Je n'ai pas su filtrer sur « {sens} "
                       f"{_nombre(seuil[1])} » — cette caractéristique n'est "
                       f"pas classable dans cette famille. La liste est donc "
                       f"complète, pas filtrée."]
