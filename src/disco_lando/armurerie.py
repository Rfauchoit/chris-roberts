...
def objets_au_seuil(con: sqlite3.Connection, query: str, *,
    ...
    **Deux vocabulaires, et il faut les deux.** `_weapon_filter` connaît la
    classe et le genre (balistique, gatling) ; il ne connaît **pas** les
    familles du catalogue. Mesuré le 2026-08-16 en écrivant `repli.py` :
    « quelles armes **FPS** font plus de 176 dps » rendait le *Helix II
    Mining Laser* et le *SF7E Cannon* — des armes de vaisseau — en tête
    d'une liste censée classer des armes de poing. Une réponse chiffrée et
    fausse, c'est-à-dire le pire cas.

    C'est la divergence que le projet a déjà payée sur les motifs :
    `_weapon_filter` est une liste écrite à la main **à côté** du
    vocabulaire fermé `_FAMILLES_D_OBJETS`. On applique donc les deux.

    **Une famille FPS ressort désormais vide, et c'est volontaire.**
    `MOUNTABLE` exige `flight_ready` et `mount_usable`, qui valent **0 sur
    les 321 armes FPS** dotées d'un DPS et sur les 58 pistolets : ce sont
    des drapeaux de montage **sur vaisseau**, comme on l'avait déjà mesuré
    sur les boucliers et refroidisseurs au sprint 38. Cet outil n'a donc
    jamais su classer une arme de poing. Ne rien rendre le **dit** ; rendre
    des canons de vaisseau sous le mot « armes FPS » le cachait. Ouvrir la
    population demande son propre chantier — les valeurs qui apparaissent
    alors sont aberrantes (341 833 DPS pour un lance-roquettes, un pistolet
    jouet en tête) et se traitent comme le PyroBurst, pas d'un trait de
    plume. Consigné au backlog.
    """
    ...
    where, args, famille = _weapon_filter(query)
    deja_fr = False
    if (detectee := detect_famille_objets(query)) is not None:
        libelle_famille, clause, _mode = detectee
        where = f"({where}) AND ({clause})"
        # **Le libellé de famille a deux sources, et une seule est
        # anglaise.** `_weapon_filter` rend « ballistic cannon », que le
        # rendu traduit en inversant l'ordre des mots ; `detect_famille_objets`
        # rend déjà du français — « armes FPS », « armes de vaisseau » —, que
        # la même inversion mutilait en « FPS armes » et « vaisseau de
        # armes ». On dit donc d'où il vient au lieu de le deviner.
        deja_fr = not famille
        famille = famille or libelle_famille

    lignes = [dict(r) for r in con.execute(
        f"SELECT i.name, i.size, i.manufacturer_name, st.{stat} valeur, "
        # **Un DPS ne vaut que le temps qu'on peut le tenir.** Le Boomtube
        # publie 341 833 dégâts par seconde et porte **une** roquette : il
        # tire 0,12 seconde avant d'être à sec. Le chiffre décrit une
        # cadence que l'arme ne soutient sur aucune seconde entière.
        # Remarque de l'utilisateur, 2026-08-21 : « 500 coups par minute,
        # combien y a-t-il de balles dedans ? »
        #
        # La **réserve** borne le tir, et le **rechargement** borne la
        # reprise. Ce commentaire a affirmé que le second n'était pas
        # publié : c'était faux, il dormait dans le fichier d'objet et non
        # dans l'index. Les deux sont désormais lus, et on rend la valeur
        # calculée à côté de la brute au lieu d'inventer un plafond — le §7
        # interdit le second, pas le premier.
        f"       st.ammo_capacity munitions, st.rounds_per_minute cadence, "
        f"       st.reload_seconds recharge, i.type genre, st.alpha alpha "
        f"FROM items i JOIN item_stats st ON st.item_uuid = i.uuid "
        f"{MOUNTABLE} "
        f"WHERE i.name IS NOT NULL AND st.{stat} IS NOT NULL AND {where} "
        f"  AND st.{stat} {seuil} ? "
    ...
    if not lignes:
        raise NotFound(query)
    for ligne in lignes:
        munitions, cadence = ligne.pop("munitions"), ligne.pop("cadence")
        genre = ligne.pop("genre")
        recharge, alpha = ligne.pop("recharge"), ligne.pop("alpha")
        # Seulement là où la question porte sur un débit : la durée de tir
        # n'éclaire pas une masse ni une portée.
        tir = (munitions / cadence * 60
               if stat == "dps" and munitions and cadence else None)
        ligne["secondes_de_tir"] = round(tir, 2) if tir else None
        # **Le débit qu'on tient réellement**, chargeur compris. Le
        # Boomtube tombe de 341 833 à 25 321 : il tire 0,12 s puis passe
        # 1,5 s à recharger. Le Vendetta, lui, tire soixante secondes — sa
        # recharge ne pèse presque rien, et c'est bien ce qu'on veut voir.
        # **Seulement les armes FPS.** Pour une arme de vaisseau,
        # `ammo_capacity` est la **réserve entière** et non un chargeur —
        # règle déjà payée par « dégâts par chargeur », qui était faux là
        # -bas. On ne recharge pas un canon en vol, on se réarme à quai :
        # le mot et le calcul n'ont pas de sens. Et la mesure le confirme,
        # les 197 armes de vaisseau valent 3 uniformément.
        ligne["dps_avec_recharge"] = (
            round(alpha * munitions / (tir + recharge), 1)
            if tir and recharge and alpha and genre == "WeaponPersonal" else None)
    return {"stat": stat, "libelle": libelle_stat, "unite": unite,
            "seuil": seuil, "valeur": valeur, "famille": famille,
            "famille_deja_fr": deja_fr,
...
def combien_y_a_t_il(con: sqlite3.Connection, query: str) -> dict[str, Any]:
    ...
    for mots, (libelle, sql) in _COMPTAGES.items():
        if any(m in norm for m in mots):
            reponse = {"quoi": libelle,
                       "combien": con.execute(sql).fetchone()[0]}
            # **Un compte de trois ne se dit pas sans les nommer.** Rejeu du
            # journal, 2026-08-21 : « quelles sont les autres étoiles à part
            # Pyro » ne trouvait rien, et « combien y a-t-il de systèmes »
            # rendait « 3 » — exact et inutilisable. En dessous d'une
            # douzaine, la liste **est** la réponse ; au-delà, le compte l'est
            # (personne ne veut les 316 vaisseaux énumérés).
            if reponse["combien"] <= 12 and (noms := _NOMS.get(mots)):
                reponse["noms"] = [r[0] for r in con.execute(noms)]
            return reponse
    raise NotFound(query)


#: Le comptage qui sait aussi se nommer. Seul le systeme s'y prete
#: aujourd'hui — les autres familles se comptent par centaines.
_NOMS: dict[tuple[str, ...], str] = {
    ("systeme", "etoile"): ("SELECT system_name FROM starmap "
                   "WHERE system_name IS NOT NULL GROUP BY 1 "
...
_COMPTAGES: dict[tuple[str, ...], tuple[str, str]] = {
    ...
    # « Étoile » est le mot que le jeu affiche pour un système — la fiche de
    # Pyro commence par « une étoile ». Le joueur reprend ce mot-là.
    ("systeme", "etoile"): (
...
# Le **rayon de boutique** (`items.rayon`, tiré de `displayType`) est la
# catégorie sous laquelle CIG range l'objet en magasin. Il dit ce que les
# tags ne disent pas — mesuré : sur les 202 armes de vaisseau, les tags en
# classent **24** (les gatlings) et le rayon **193**, sans une seule
# contradiction quand les deux répondent. Sur les armes FPS c'est
# l'inverse : les tags sont plus fins (une LMG y est une mitrailleuse, le
# rayon la range avec les fusils), mais ils laissent 115 armes en « autre »
# dont le rayon en rattrape 71 — la mêlée et les gadgets, qu'aucun tag ne
# nomme. Les deux sources sont donc **complémentaires**, et l'ordre est
# celui de la finesse : les tags d'abord, le rayon en repli.
_RAYON_FR: dict[str, str] = {
    # armes de vaisseau
    "Cannon": "canon", "Repeater": "répéteur", "Gatling": "gatling",
    "Scattergun": "scattergun", "Rocket": "lance-roquettes",
    "Beam": "faisceau", "Mining Laser": "laser de minage",
    "Salvage Beam": "faisceau de récupération",
    "Scraper Beam": "faisceau de grattage",
    "Missile Rack": "rack à missiles", "Bomb Rack": "rack à bombes",
    "Ordnance": "munition", "Mine": "mine", "Area Weapon": "arme de zone",
    "Mount (Fixed)": "affût fixe", "Mount (Fluid)": "affût fluide",
    # armes FPS
    "Rifle": "fusil", "Pistol": "arme de poing", "SMG": "mitraillette",
    "Shotgun": "fusil à pompe", "Sniper": "sniper", "Grenade": "grenade",
    "Melee Weapon": "arme de mêlée", "Shouldered": "arme d'épaule",
    "Gadgets": "gadget", "Gadget": "gadget", "Device": "appareil",
    "Personal Flare": "fusée éclairante", "Special": "arme spéciale",
    # accessoires d'arme
    "Optics Attachment": "optique", "Barrel Attachment": "bouche",
    "Underbarrel Attachment": "sous-canon", "Magazine": "chargeur",
    "Utility": "utilitaire",
    # équipement personnel
    "Helmet": "casque", "Armor: Core": "plastron", "Armor: Legs": "jambières",
    "Armor: Arms": "brassards", "Undersuit": "combinaison",
    "Backpack": "sac à dos", "Jacket": "veste", "Shirt": "haut",
    "Pants": "pantalon", "Shoes": "chaussures", "Gloves": "gants",
    "Hat": "chapeau", "mobiGlas": "mobiGlas",
    # composants de vaisseau
    "Power Plant": "générateur", "Cooler": "refroidisseur", "Radar": "radar",
    "Shield Generator": "bouclier", "Quantum Drive": "moteur quantique",
    "Jump Module": "module de saut", "Fuel Tank": "réservoir",
    "Flight Blade": "lame de vol",
    "Quantum Enforcement Device": "brouilleur quantique",
    # divers
    "Liveries": "livrée", "Food": "nourriture", "Drink": "boisson",
    "Consumer Goods": "bien de consommation", "Novelty": "babiole",
    "Decoration": "décoration",
}


def rayon_fr(rayon: str | None) -> str | None:
    """Le rayon de boutique en français, ou None s'il n'y en a pas.

    Le vocabulaire amont est fermé — 62 valeurs mesurées — mais un patch
    peut en ouvrir une : on rend alors la valeur d'origine plutôt qu'un
    vide, comme `_constructeur()` le fait déjà pour les marques inconnues.
    """
    if not rayon:
        return None
    return _RAYON_FR.get(rayon, rayon.lower())


def _classe_d_arme(tags: str | None, nom: str | None,
                   rayon: str | None = None) -> str:
    bas = f"{tags or ''} {nom or ''}".lower()
    for cle, libelle in _CLASSES_D_ARME:
        if cle in bas:
            return libelle
    # Les tags ne nomment ni la mêlée, ni les gadgets, ni les fusées
    # éclairantes : sans ce repli, 115 armes FPS sur 458 se rangeaient
    # ensemble sous « autre », ce qui est le contraire d'un catalogue.
    return rayon_fr(rayon) or "autre"
...
#
# **Les motifs sont au pluriel strict, et c'est structurel.** « Les
# répéteurs » est un catalogue, « le répéteur CF-337 » une fiche — le
# commentaire du routeur le disait déjà (« le singulier reste aux fiches et
# aux composants de vaisseau »), mais il le tenait par un *second* motif
# écrit à la main, lui au pluriel. Depuis que le routeur déduit son motif
# d'ici, cette liste est seule à porter la règle : y laisser un `armes?`
# rendait « combien de balles dans **une arme de vaisseau** balistique » au
# catalogue au lieu du classement, question mesurée par le cahier.
_FAMILLES_D_OBJETS: tuple[tuple[str, str, str, str], ...] = (
    ...
    # « Canons » sort de ce motif générique : le rayon distingue désormais
    # les 95 vrais canons des 49 répéteurs, et la première correspondance
    # gagnant, la famille large avalait la précise.
    ("armes de vaisseau", r"\barmes de vaisseaux?\b",
    ...
    # **Les familles que seul le rayon de boutique sait nommer.** Le `type`
    # amont confond ce que le joueur distingue : les 202 `WeaponGun` sont
    # canons, répéteurs, gatlings et scatterguns, et les 458
    # `WeaponPersonal` vont du pistolet à l'arme de mêlée. Mesuré avant
    # branchement, ces dix questions rendaient toutes « je n'ai pas
    # compris ».
    #
    # Le motif exige le **pluriel strict** : « les répéteurs » est un
    # catalogue, « le répéteur CF-337 » est une fiche. C'est le même
    # partage que les familles ci-dessus, et il évite qu'un poids de 4,0
    # détourne une question nommant un objet précis.
    ("répéteurs", r"\brepeteurs\b|\brepeaters\b",
     "rayon = 'Repeater'", "taille"),
    ("canons de vaisseau", r"\bcanons de vaisseaux?\b",
     "rayon = 'Cannon'", "taille"),
    ("gatlings", r"\bgatlings\b", "rayon = 'Gatling'", "taille"),
    ("scatterguns", r"\bscatterguns\b", "rayon = 'Scattergun'", "taille"),
    ("lance-roquettes de vaisseau",
     r"\blance[ -]?roquettes de vaisseaux?\b", "rayon = 'Rocket'", "taille"),
    ("pistolets", r"\bpistolets\b|\barmes de poing\b",
     "rayon = 'Pistol'", "marque"),
    ("fusils à pompe", r"\bfusils a pompe\b|\bshotguns\b",
     "rayon = 'Shotgun'", "marque"),
    ("snipers", r"\bsnipers\b|\bfusils de precision\b",
     "rayon = 'Sniper'", "marque"),
    ("mitraillettes", r"\bmitraillettes\b|\bsmgs\b",
     "rayon = 'SMG'", "marque"),
    ("armes de mêlée", r"\barmes de melee\b|\bcorps a corps\b",
     "rayon = 'Melee Weapon'", "marque"),
    # `Char_Clothing_Torso_1` mélange 484 vestes et 32 combinaisons : le
    # type ne les sépare pas, le rayon si. « Combinaisons » reste au
    # classement d'équipement, qui répond déjà — on n'ouvre que ce qui
    # n'avait pas de porte.
    ("vestes", r"\bvestes\b|\bblousons\b", "rayon = 'Jacket'", "marque"),
)
...
def question_hors_famille(question: str, libelle: str) -> str:
    """La question privée des mots qui nomment sa famille d'objets.

    Un garde-fou qui cherche « vaisseau » dans « quels **canons de
    vaisseau** » se saborde : le mot appartient au nom de la famille, pas
    au sujet de la question. On retire donc ce que le motif a apparié
    avant de juger le reste — même mécanique que `_mots_d_intention`, qui
    retire l'intention avant de chercher l'entité.
    """
    norm = normalize(question or "")
    for autre, motif, _clause, _mode in _FAMILLES_D_OBJETS:
        if autre == libelle:
...
def _groupe_de(ligne: dict[str, Any], mode: str) -> str:
    ...
    if mode == "marque":
        # Une famille déjà filtrée sur un rayon unique n'a plus rien à
        # tirer d'un groupe par classe — il serait constant. Ce que le
        # joueur compare alors, c'est la marque.
        # `_constructeur` rend None sur « <= PLACEHOLDER => », que 94 objets
        # portent : retomber sur la valeur brute afficherait le gabarit
        # comme un nom de marque — la règle des titres de mission, dans un
        # champ où on ne l'avait pas cherchée.
        return str(ligne.get("marque") or "marque inconnue")
...
def catalogue_objets(con: sqlite3.Connection, query: str, *,
                     ...
                     mode: str = "taille",
                     seuil_ignore: tuple[str, float] | None = None,
    ...
    lignes = [dict(r) for r in con.execute(
        f"SELECT name, subtype, size, manufacturer_name, tags, rayon "
        f"FROM items "
    ...
    if mode == "marque":
        # Résolu une fois par code, pas une fois par ligne : le catalogue
        # des pistolets compte 63 objets pour une poignée de marques.
        codes = {ligne.get("manufacturer_name") for ligne in lignes}
        noms = {code: _constructeur(con, code) for code in codes if code}
        for ligne in lignes:
            ligne["marque"] = noms.get(ligne.get("manufacturer_name"))
    ...
    return {"famille": famille + (f" de taille {taille}"
            ...
            # Ligne 1 de la grille : une contrainte qu'on ne sait pas
            # honorer se **dit**, elle ne s'oublie pas.
            "seuil_ignore": seuil_ignore,
