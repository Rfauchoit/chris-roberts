...
from . import (analyste, cockpit, config, context, erreurs, meta, render,
               repli, resultats, router)
...
from .router.analyse import detect_nombre_de_combats
...
def _est_une_declinaison(nom: str, noms: Iterable[str], question: str) -> bool:
    """Ce nom est-il la déclinaison d'un autre, que rien dans la question ne réclame ?

    Vrai quand il reprend **tous** les mots d'un autre nom de la liste et en
    ajoute, sans qu'aucun des mots ajoutés n'apparaisse dans ce que le joueur
    a tapé. C'est la règle « le nom nu désigne le modèle de base ».
    """
    from .normalize import normalize

    tape = set(normalize(question).split())
    mots = set(normalize(nom).split())
    for autre in noms:
        base = set(normalize(autre).split())
        # **Le sur-ensemble, pas le préfixe.** Premier essai : « le nom de la
        # déclinaison commence par celui du modèle ». Il attrapait *Aegis
        # Gladius Valiant* et ratait *CQ7 "Infantry" Rifle*, où le jeu insère
        # le qualificatif **au milieu** — la forme la plus répandue chez les
        # armes. Un candidat qui reprend tous les mots d'un autre et en ajoute
        # est une déclinaison de celui-là, où que les mots ajoutés tombent.
        if not base or base == mots or not base < mots:
            continue
        # Un mot ajouté que le joueur a tapé rend la déclinaison légitime :
        # « gladius valiant » demande bien le Valiant.
        if not (mots - base) & tape:
            return True
    return False
...
def variantes_ambigues(con: sqlite3.Connection, call: router.ToolCall,
    ...
    from .normalize import normalize as normalize_terme

    # **Le nom tapé au caractère près n'est pas une ambiguïté.** Rejeu du
    # journal, 2026-08-21 : « P6-LR Sniper Rifle » — le nom officiel, écrit
    # tel quel — rendait « tu parles duquel ? P6-LR "Gideon" Sniper Rifle, ou
    # P6-LR Sniper Rifle ? », trois fois. Le joueur a déjà répondu à la
    # question qu'on lui repose. C'est la règle « un candidat unique ne pose
    # aucune question », vue par l'autre bout : ici le candidat est désigné
    # par son nom, pas par sa solitude.
    if any(normalize_terme(c.name) == normalize_terme(terme)
    ...
    # **Le fragment expliqué bat le score — sur la question entière, pas sur
    # le terme extrait.** Journal du 2026-08-19 : « combien coûte un panther
    # repeater » demandait « Prowler Panthera Livery, CF-337 Panther
    # Repeater, ou … ? ». Le routeur n'avait transmis que « panther » —
    # « repeater » est un mot d'intention, mangé par `_mots_d_intention` —
    # et au niveau du terme, la livrée est une lecture légitime. C'est la
    # règle du P8AR : ce qui départage se cherche dans la **question**.
    #
    # Le critère est **relatif** : on garde les candidats qui expliquent le
    # maximum de mots utiles. Absolu, il aurait vidé « les SCU d'un
    # Avenger » — « scu » est inexpliqué par toutes les variantes, à
    # égalité, et ce n'est pas à lui de trancher.
    from .normalize import MOTS_GRAMMATICAUX, normalize

    def _inexpliques(candidat) -> int:
        utiles = [m for m in normalize(question or terme).split()
                  if m not in MOTS_GRAMMATICAUX]
        phrase = " ".join(utiles)
        return min(len(mots_inexpliques(phrase, candidat.alias)),
                   len(mots_inexpliques(phrase, candidat.name)))

    en_lice = [c for c in resolution.candidates if c.score >= plancher]
    plancher_explication = min((_inexpliques(c) for c in en_lice), default=0)

    for candidat in resolution.candidates:
        if candidat.score < plancher or candidat.entity_id in vus:
            continue
        if _inexpliques(candidat) > plancher_explication:
            continue
        ...
        if (candidat.name.startswith("MineableRock_")
            ...
            continue
        # **Le nom nu désigne le modèle de base, pas ses livrées.** Rejeu du
        # journal, 2026-08-21 : sur les 24 désambiguïsations du corpus, **15
        # font figurer une livrée dans le choix** et 13 tombent sur le duel —
        # « qui gagne entre un wolf et un arrow » proposait *Kruger L-21 Wolf
        # Wikelo Sneak Special* et *Kruger L-22 Alpha Wolf Wikelo War
        # Special* à côté des deux vrais modèles.
        #
        # C'est la règle du P8-AR — « quand je dis "le p8ar" je parle
        # forcément de l'arme » — transposée au vaisseau, et elle ne peut
        # pas se poser sur la seule comparaison des textes : les livrées
        # portent de **vrais** armements différents, donc leur duel diffère
        # réellement, et le critère « la réponse changerait » les garde à
        # juste titre. Ce qui les disqualifie n'est pas leur réponse, c'est
        # que le joueur ne les a pas nommées.
        #
        # Le critère est donc le **préfixe plus le silence de la question** :
        # on n'écarte un candidat que si son nom prolonge celui d'un autre
        # candidat **et** que les mots ajoutés sont absents de ce que le
        # joueur a tapé. « Gladius Valiant » demandé explicitement reste ;
        # *Aegis Avenger Titan* et *Aegis Avenger Stalker* ne se prolongent
        # pas l'un l'autre et restent tous les deux — c'est un vrai choix,
        # et leurs 8 / 0 SCU sont précisément le cas qui a fait écrire cette
        # fonction.
        # **Et un cosmetique n est pas un choix.** Le resolveur le relegue
        # deja au classement (voir `resolver._uuids_cosmetiques`) ; ici il
        # faut le **retirer** de la liste, sinon il reste dans la fenetre de
        # tolerance et « ou trouver les panther » continue de proposer
        # *Prowler Panthera Livery* a cote du repeteur.
        if candidat.cosmetique and any(not c.cosmetique
                                       for c in resolution.candidates):
            continue
    ...
    def sans_les_noms(texte: str) -> str:
        ...
        texte = re.sub(r"(?m)^_?Mêmes [^\n]*$", "", texte)
        # **Et le lien vers le cockpit s'efface aussi**, pour exactement la
        # même raison. Il porte le nom de la variante en paramètre de
        # recherche — et **encodé** (`P6-LR+Sniper+Rifle`), donc le masquage
        # des noms ci-dessus ne le voit pas. Mesuré le 2026-08-14 : « la
        # recette du P6-LR » s'est remise à demander « tu parles duquel ? »
        # entre quatre variantes dont la recette est identique, sur la seule
        # foi de cette ligne. Le lien mène à la même page pour toutes : il ne
        # dit rien qui aide à choisir.
        # `sans_le_lien` referme aussi les trous que ce retrait laisse.
        # `_absorber` plus bas repose sur une relation de **préfixe** — « le
        # court est le long en moins renseigné » — et une ligne vide en fin
        # de texte la casse : « X.\n\n » n'est pas contenu dans « X. Le
        # blueprint…\n\n ». C'est exactement ce qui s'est produit en ajoutant
        # le lien du cockpit, et le symptôme était trompeur : les quatre
        # variantes du P6-LR se remettaient à demander « tu parles duquel ? »
        # alors que la ligne fautive avait bien été retirée. Le retrait de
        # « Mêmes … » portait le même défaut en germe ; il ne s'était jamais
        # déclenché parce que cette phrase n'est jamais la dernière.
        return cockpit.sans_le_lien(texte)
    ...
    if len({texte for _, texte in pleins}) > 1:
        ...
        # **Et la declinaison suit exactement la meme regle que l accessoire.**
        # « Un CQ7 » designe le CQ7 Rifle, pas le CQ7 "Infantry" Rifle — c est
        # la regle du P8-AR, que l utilisateur a posee pour *toutes* les armes,
        # et elle vaut pour le vaisseau (« un wolf ») comme pour l arme. Elle
        # se pose **ici** et pas dans le vivier de candidats : une declinaison
        # ecartee de la question garde sa reponse, et le rendu la nomme —
        # « CQ7 "Infantry" Rifle a sa propre reponse ». La retirer plus haut
        # la faisait disparaitre aussi de `sans_reponse`, ce qui effacait des
        # objets qui existent.
        noms_pleins = [nom for nom, _ in pleins]
        def _hors_question(nom: str) -> bool:
            return est_un_accessoire(nom) or _est_une_declinaison(
                nom, noms_pleins, question or terme)

        armes = [(nom, texte) for nom, texte in pleins
                 if not _hors_question(nom)]
        ecartes = [nom for nom, _ in pleins if _hors_question(nom)]
...
def certitude_insuffisante(con: sqlite3.Connection, call: router.ToolCall,
    ...
    # **Le nom tapé au caractère près ne se fait pas redemander.** La règle
    # existait dans `variantes_ambigues` depuis le 2026-08-21 et n'avait pas
    # été portée ici, où vit l'autre chemin de doute. Trouvé en auditant
    # `rendu-et-conversation` : « les composants du **Anvil F7C-S Hornet
    # Ghost Mk I** » — le nom officiel, écrit tel quel — demandait « je ne
    # suis pas sûr, tu veux dire Anvil F7C Hornet Mk I, Anvil F7C-S Hornet
    # Ghost Mk I, ou Mk II ? ».
    #
    # Le joueur a déjà répondu à la question qu'on lui repose. Et le pire
    # suivait : recopier exactement le nom proposé rendait « je n'ai pas
    # compris » — c'est la famille documentée « répondre exactement ce qu'on
    # a proposé était la seule chose à ne pas faire », quatre cas au journal.
    from .normalize import normalize as _normalise

    if any(_normalise(c.name) == _normalise(terme)
           for c in resolution.candidates):
        return []
    ...
    utiles = [c for c in resolution.candidates if not c.cosmetique] or None
    for candidat in resolution.candidates:
        if candidat.score < MIN_ENTITY_SCORE:
            continue
        if candidat.name is None or candidat.name in vus:
            continue
        # Ni un cosmétique ni une déclinaison que la question ne réclame pas :
        # les deux règles de `variantes_ambigues`, au second point de choix.
        # « Combien coûte un prospector » proposait *MISC Prospector
        # Schematic* — un tableau mural — à côté du vaisseau.
        if utiles is not None and candidat.cosmetique:
            continue
        if _est_une_declinaison(candidat.name,
                                [c.name for c in resolution.candidates],
                                question or terme):
            continue
...
def _vaisseaux_de_combat(outil: str | None, data: Any) -> tuple[str, ...]:
    ...
    if outil in ("peut_detruire", "bataille", "simuler_duel"):
...
_MOTS_DE_DUEL = frozenset({
    ...
    "matchup", "matchups",
    # « Sur 10 combats, combien de fois gagne le Arrow ? » : la question du
    # 2026-08-13 ne nomme **qu'un** vaisseau — l'autre vient du duel qui
    # précède. Sans ces mots, elle ne se rattachait à rien et partait à
    # l'analyste, qui répondait à juste titre qu'il n'avait pas d'outil.
    "combat", "combats", "fois", "simulation", "simuler", "simule",
    "affrontement", "affrontements", "statistiquement",
    # **« Et contre un hammerhead ? » ne contenait aucun mot de duel.** Elle
    # n'entrait donc pas dans cette reprise et finissait au repli — « je n'ai
    # pas saisi ce que tu veux savoir sur Aegis Hammerhead » — après un duel
    # que le joueur venait de lire. Le mot n'ouvre rien tout seul : cette
    # fonction ne s'exécute que si le tour précédent a rendu une paire.
    "contre", "face", "versus", "vs",
})


#: Ce qui, dans une reprise de duel, réclame la simulation plutôt que le
#: matchup. Serré : « combat » nu ne suffit pas — « leur puissance de
#: combat » parle encore du matchup —, il faut un **compte**.
_VOCABULAIRE_DE_SIMULATION = re.compile(
    r"\bsur \d+ (?:combats?|duels?|affrontements?|matchs?)\b"
    r"|\b\d+ (?:combats?|duels?|affrontements?)\b"
    r"|\bcombien de fois\b|\bsimul\w+"
    r"|\bfois sur (?:dix|vingt|cent|\d+)\b|\bstatistiquement\b")
...
def _reprise_de_duel(con: sqlite3.Connection,
    ...
    # celui-ci.
    #
    # **Cette garde ne se déclenchait jamais sur une phrase longue.** Elle
    # exigeait `not mots_inexpliques(question, tiers.alias)`, c'est-à-dire
    # que le nom du tiers explique la question **entière** — impossible dès
    # qu'il y a plus de trois mots. Mesuré au rejeu du journal, 2026-08-21 :
    # après un duel Gladius/Arrow, « Scorpius et Hurricane, prends bien en
    # compte toutes les statistiques » resservait Gladius contre Arrow, alors
    # que la phrase nomme deux autres vaisseaux.
    #
    # Le bon critère est celui que la ligne du dessus emploie déjà pour la
    # paire : le vaisseau est **nommé** si un mot de son nom est dans la
    # question. Et ce mot doit lui être propre — sinon « Aegis » suffirait à
    # faire passer l'Avenger pour un tiers dans un duel de Gladius.
    mots_de_la_paire = set()
    for nom in (a, b):
        mots_de_la_paire |= set(normalize(nom).split())
    etrangers: list[str] = []
    tous = resolve_entity(con, question, entity_types=("ship",),
                          limit=12).candidates
    for candidat in tous:
        if candidat.score < 85.0 or candidat.name in (a, b):
            continue
        # **Et le mot qui identifie doit peser au moins quatre lettres.**
        # « Tu as oublie que **c** etait des vaisseaux 2 joueurs » faisait
        # sortir *C.O. Mustang Beta* a 85,5 par le seul jeton « c », et la
        # correction du duel se transformait en duel de Mustang. C'est la
        # regle « un alias de trois lettres ou moins ne vaut que par
        # egalite », transposee au mot qui **designe** le tiers.
        propres = {m for m in (set(normalize(candidat.name).split())
                               - context._LIANTS - mots_de_la_paire)
                   if len(m) >= 4}
        if propres & mots and candidat.name not in etrangers:
            etrangers.append(candidat.name)
    # **Le nom nu designe le modele de base — et la base peut etre sous le
    # seuil.** « Et contre un hammerhead ? » rendait *Aegis Hammerhead 2949
    # Best In Show Edition* : l edition sort a 85,5 par l etage `fts` quand
    # l *Aegis Hammerhead* lui-meme n est qu a 81,5 par `token`. Comparer
    # apres le seuil ne pouvait donc pas les departager. On cherche le
    # modele dont la declinaison retenue derive, **a tous les scores**, et
    # on lui rend sa place.
    def _le_modele(nom: str) -> str:
        mots_du_nom = set(normalize(nom).split())
        socles = [c.name for c in tous
                  if set(normalize(c.name).split()) < mots_du_nom
                  and set(normalize(c.name).split()) & mots]
        return min(socles, key=len) if socles else nom

    etrangers = list(dict.fromkeys(_le_modele(n) for n in etrangers))

    # **Deux vaisseaux étrangers nommés, c'est un duel neuf — pas un abandon.**
    # Rendre `None` ici laissait le routeur reprendre la main *avec le
    # contexte*, et « Scorpius et Hurricane, prends bien en compte toutes les
    # statistiques » sortait alors la fiche de l'**Aegis Gladius** : le
    # vaisseau du duel précédent, sur une question qui nomme les deux autres.
    # Pire que le défaut d'origine, qui resservait au moins un duel.
    if len(etrangers) >= 2:
        return router.ToolCall(tool="matchups_vaisseau",
                               args={"query": etrangers[0],
                                     "cible": etrangers[1]},
                               confidence=0.9, via="duel_rebondi")
    # Un seul étranger — « et contre un hammerhead ? » — garde **mon**
    # vaisseau et change d'adversaire. C'est la lecture naturelle : le
    # premier bord est celui sur lequel le joueur interroge depuis le début.
    # Laissé au routeur, ce tour finissait au repli : « je n'ai pas saisi ce
    # que tu veux savoir sur Aegis Hammerhead ».
    if len(etrangers) == 1:
        return router.ToolCall(tool="matchups_vaisseau",
                               args={"query": a, "cible": etrangers[0]},
                               confidence=0.9, via="duel_rebondi")

    # Une reprise qui demande un **compte de combats** ne veut pas le
    # matchup : elle veut la simulation. C'est la seule branche où le duel
    # repris change d'outil, et c'est la question du journal du 2026-08-13.
    if _VOCABULAIRE_DE_SIMULATION.search(normalize(question)):
        return router.ToolCall(
            tool="simuler_duel",
            args={"query": a, "cible": b,
                  "passes": detect_nombre_de_combats(question)},
...
def _trop_sollicite(demandeur: str | None) -> bool:
    """Ce demandeur a-t-il dépassé son rythme d'analyse sur l'heure ?

    **Le garde-fou est non bloquant, et il ne s'applique qu'au doute.**
    C'est le seul chemin où une réponse déterministe existe déjà : la
    servir avec sa réserve est complet, alors qu'un `no_intent` freiné ne
    rendrait rien du tout — un silence, exactement ce que le projet
    refuse. Le sprint 44 le dit ainsi : « le remède n'est pas un frein,
    c'est un compteur » ; celui-ci n'est qu'une vigie en attendant ce que
    le panneau montrera.

    Import différé et échec silencieux, comme la comptabilité : une base
    de guilde absente ne doit pas décider du sort d'une question.
    """
    if not demandeur:
        return False
    try:
        ...
        try:
            return usage.trop_sollicite(con, demandeur)
        finally:
            con.close()
...
def _completer_si_partiel(con: sqlite3.Connection, session: str | None,
                          ...
                          speaker: str | None = None) -> Echange | None:
    ...
    reponse = analyste.repondre(demande, session=session, demandeur=speaker)
    if not reponse:
        return None
...
def _executer(con: sqlite3.Connection, session: str | None, question: str,
    ...
    completee = _completer_si_partiel(
        con, session, question, call.tool, data, texte, speaker=speaker)
...
def _traiter(con: sqlite3.Connection, session: str | None, question: str,
    ...
    if fournisseur_force is not None:
        ...
        reponse_forcee = analyste.repondre(
            a_analyser, fournisseur=fournisseur_force, session=session, demandeur=speaker)
    ...
    if complement is not None:
        ...
        # **Ce qui manque n'est pas toujours une entité.** « On est combien ? »
        # attend un nombre, et le projet interdit précisément qu'un nombre nu
        # en soit une — « 3 » sort *Origin 300i*, « 100 » sort *Origin 100i*.
        # Passer par `resolve_entity` rendrait donc un vaisseau ou rien. Le
        # type `nombre` est le seul qui ne va pas au résolveur.
        if complement.complete_types == ("nombre",):
            import re as _re

            lu = _re.search(r"\b(\d{1,2})\b", normalize(question))
            mots = {"un": 1, "une": 1, "deux": 2, "trois": 3, "quatre": 4,
                    "cinq": 5, "six": 6, "sept": 7, "huit": 8, "neuf": 9,
                    "dix": 10, "seul": 1, "solo": 1}
            valeur = int(lu.group(1)) if lu else next(
                (v for m, v in mots.items() if m in normalize(question).split()),
                None)
            if valeur:
                args = dict(complement.args)
                args[complement.complete_avec] = valeur
    ...
    if reprises:
        ...
        completee = _completer_si_partiel(
            ...
            limites=limites_reprises, speaker=speaker)
        if completee is not None:
            return completee
        return Echange(
    ...
    if duel_repris is not None:
        ...
        if format_demande and analyste.disponible():
            reponse = analyste.repondre(
                _prompt_analyste(session, question, speaker), session=session, demandeur=speaker)
            if reponse:
                return _echange_d_analyste(
                    con, session, question, reponse, question,
        ...
        if echange is not None:
            if (precedent is not None and precedent.reponse
                ...
                reponse = analyste.repondre(
                    ...
                    session=session, demandeur=speaker)
                if reponse:
                    return _echange_d_analyste(
                        con, session, question, reponse, question,
    ...
    if (call is not None and incapable and analyste.disponible()
            and not _trop_sollicite(speaker)):
        reponse = analyste.repondre(
            _prompt_analyste(session, question, speaker), session=session, demandeur=speaker)
        if reponse:
            return _echange_d_analyste(
    ...
    if call is None:
        ...
        if analyste.disponible():
            ...
            if reponse:
                return _echange_d_analyste(
                    con, session, question, reponse, question)
        # **Le silence ne vaut que si on ne nous parle pas.** Demande de
        # l'utilisateur (2026-08-16). `meta` a déjà pris l'aparté, le
        # bavardage d'atelier et le hors-univers : ce qui arrive ici est
        # adressé au bot. Si la question a **nommé** quelque chose, on dit
        # ce qu'on sait en dire plutôt que de fermer la porte.
        #
        # L'échec reste consigné tel quel : la question n'a pas trouvé son
        # outil, et c'est ce que `unanswered` doit voir pour que le journal
        # reste la feuille de route. Le repli change ce que le joueur lit,
        # pas ce que le projet mesure.
        # **Les deux corrections se composent, elles ne s'opposent pas.** Le
        # repli sait quoi dire quand la question a **nommé** quelque chose ;
        # le renvoi au cockpit sert quand elle n'a rien nommé du tout, et que
        # « je n'ai pas compris » laisserait le joueur deviner ce qu'il
        # aurait fallu demander. Le plus spécifique d'abord.
        axes = repli.proposer(con, question)
        if axes is not None:
            return Echange(question=question, answered=False,
                           echec="no_intent", tool="repli", via="repli",
                           texte=render.render("repli", axes), data=axes)
        aveu = ("Je n'ai pas compris la question. Nomme un vaisseau, une "
                "arme, un lieu ou une mission et je te dis ce que j'en sais.")
        guide = cockpit.url("guide")
        if guide:
            aveu += (f"\n\n{cockpit.PREFIXE} — "
                     f"{cockpit._PROMESSES['guide']} : {guide}*")
    ...
    try:
        data = router.execute(con, call)
    except NotFound as absence:
        # **Une absence expliquée n'est pas une absence.** Quand l'outil sait
        # *pourquoi* la donnée manque — le jeu ne nomme que 6 propulseurs
        # distincts, il ne publie aucune caractéristique propre aux 77
        # radars —, cette phrase est la réponse. Elle mourait dans le
        # message d'exception, remplacée par « je n'ai pas trouvé la
        # donnée » : le joueur croyait à une lacune du bot, sur une
        # question à laquelle on répond très bien. Et on n'appelle pas
        # l'analyste pour ça — il chercherait une donnée qu'on sait absente.
        if getattr(absence, "explication", None):
            return Echange(question=question, answered=True,
                           texte=absence.explication, tool=call.tool,
                           args=call.args, confidence=call.confidence,
                           via=call.via)
    ...
    # **Une question de calcul à laquelle on répond par une fiche doit le
    # dire.** Le vocabulaire d'analyse passe devant le routeur — mais
    # seulement quand l'analyste est joignable. Éteint, « donne-moi le ratio
    # DPS / coût de la balle entre un P4-AR et un P6-LR » retombe sur
    # `get_item_stats` et rend « **P4-AR Rifle** a 40 balles » : exact, et à
    # côté de la question, sans que rien ne le signale. Mesuré au rejeu du
    # journal du 2026-08-21 — le pire cas, puisque la réponse a l'air
    # complète.
    if analyste.question_d_analyse(question) and not analyste.disponible():
        texte += ("\n\n---\n*Le calcul demandé (ratio, moyenne, médiane…) "
                  "n'est pas un outil du bot : ceci est la fiche la plus "
                  "proche. L'analyste, qui sait calculer librement sur la "
                  "base, n'est pas joignable en ce moment.*")
    # Le renvoi vers le cockpit est un pied de page : les deux ajouts
    # ci-dessus l'auraient laissé au milieu de la réponse.
    texte = cockpit.remettre_en_queue(texte)
    completee = _completer_si_partiel(
        con, session, question, call.tool, data, texte, speaker=speaker)
    if completee is not None:
        return completee
...
def traiter(con: sqlite3.Connection, session: str | None, question: str,
    ...
    if session and echange.texte:
        ...
        # **Un outil qui répond est le dernier sujet, même s'il n'a rien
        # retenu.** Toutes les branches métier ne posent pas leur contexte —
        # `jalons_de_qualite` répond sans retenir — et l'enveloppe reportait
        # alors l'outil **et** l'entité du tour d'avant. Mesuré au rejeu du
        # journal, 2026-08-21 : après « combien coûte un arrow » puis
        # « combien de balles pour tuer avec un F55 LMG », la reprise « et
        # pour le p6lr ? » repartait chez `get_price` et rendait le prix du
        # fusil — une réponse exacte à une question que personne n'a posée.
        #
        # Le report ne vaut que pour un tour **sans outil** : un « merci »,
        # une colère, une phrase de méta. Là, garder le sujet est ce qui
        # permet à la reprise suivante de retrouver son fil.
        if mis_a_jour and apres is not None:
            entite, outil = apres.entite, apres.outil
        elif echange.tool:
            entite, outil = echange.entity, echange.tool
        else:
            entite = avant.entite if avant else None
            outil = "conversation"
