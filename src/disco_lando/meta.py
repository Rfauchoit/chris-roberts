...
_ATELIER = re.compile(
    ...
    r"\bmise en prod\w*\b|\bhotfix\b|\btroll\w*\b|\bles modifs\b")

# **Et le joueur parle parfois du bot lui-même.** Rejeu du journal du
# 2026-08-21 : treize tours de commentaire sur mon fonctionnement recevaient
# « je n'ai pas compris la question » — c'est-à-dire lui reprocher d'avoir
# mal parlé alors qu'il décrivait précisément ce qui n'allait pas. Deux
# d'entre eux **résolvaient une entité au hasard** : « il est gris » sortait
# *G-2 Helmet Grey*, « il doit vérifier si la question a une réponse possible »
# sortait la mission *Patrol to Prevent Possible Attack*.
#
# Le vocabulaire est fermé et ne recoupe aucun mot du jeu — « contexte »,
# « reset », « fenêtre de contexte », « réponses différentes » n'ont pas de
# sens dans le 'Verse. Et ce détecteur ne voit de toute façon que ce que
# l'étage 1 n'a pas su servir.
_A_MON_SUJET = re.compile(
    r"\btu (?:savais|sais que|oublies?|mens|te (?:souviens|rappelles))\b|"
    r"\bfenetre de contexte\b|\bde contexte\b|\breset\w*\b|"
    r"\bil oublie\b|\btu oublies\b|\btu as oublie\b|"
    r"\breponses? differentes?\b|\bdeux reponses\b|"
    r"\btu savais le faire\b|\bj ai menti\b|"
    r"\bquand tu parles\b|\bchaque fois que tu parles\b")

#: **Les couleurs, vocabulaire fermé.** Il ne sert qu'au cas de la couleur
#: seule et ne recoupe aucun nom d'entité pris **isolément** : « Black Prism »
#: se demande en deux mots, « le casque gris » en porte un autre. Mesuré sur
#: les 435 tours du corpus : **une seule** question est entièrement faite de
#: couleurs, et c'est celle que ce vocabulaire vise.
_COULEURS = frozenset({
    "gris", "grise", "grises", "noir", "noire", "noires", "blanc", "blanche",
    "blanches", "rouge", "rouges", "bleu", "bleue", "bleues", "vert", "verte",
    "vertes", "jaune", "jaunes", "orange", "violet", "violette", "rose",
    "marron", "beige", "dore", "doree", "argente", "argentee", "chrome",
    "chromee", "grey", "gray", "black", "white", "red", "blue",
...
def detecte(question: str, *, a_un_tour_precedent: bool = False) -> Meta | None:
    ...
    # Ici la phrase peut être longue — décrire un défaut demande des mots —
    # donc pas de `_essentiellement` : le vocabulaire est assez propre au
    # bot pour se passer de cette garde, et une remarque tronquée à trois
    # mots utiles n'existe pas.
    if _A_MON_SUJET.search(norm):
        return Meta("a_mon_sujet",
                    "Noté. Ce que je garde d'un tour à l'autre tient dans "
                    "une fenêtre de quinze minutes, et je ne réponds que "
                    "sur les fichiers du jeu installé. Repose-moi la "
                    "question et je m'y remets — ou dis « demande à "
                    "Claude » si tu veux un calcul libre sur la base.")

    # **Une couleur seule ne désigne rien, et le jeu n'indexe pas par
    # couleur.** Journal du 2026-08-12 : « quel est le meilleur loadout pour
    # mon vaisseau chromé (je me souviens plus de son nom) » puis « **il est
    # gris** ». Le joueur décrit son vaisseau ; le repli y voyait un nom et
    # proposait trois axes sur *Deo Shirt Grey*.
    #
    # La règle générale a été cherchée et **n'existe pas** : « gris » touche
    # 66 entités et n'est jamais en tête d'un nom, mais « onyx » en touche 80
    # (question légitime) et « titan » n'est jamais en tête non plus (reprise
    # légitime). Ni le compte ni la position ne séparent quoi que ce soit.
    # Ce qui se mesure, c'est le vocabulaire lui-même : sur les 435 tours du
    # corpus, **une seule** question est entièrement faite de couleurs, et
    # c'est celle-ci. On ne ferme donc que ce cas, et on rend la question
    # qui manque plutôt que le silence.
    utiles_de_la_phrase = [m for m in norm.split()
                           if m not in MOTS_GRAMMATICAUX]
    if utiles_de_la_phrase and all(m in _COULEURS for m in utiles_de_la_phrase):
        return Meta("couleur_seule",
                    "Je ne sais pas reconnaître un vaisseau ou un objet à sa "
                    "couleur — les fichiers du jeu ne l'indexent pas ainsi. "
                    "Donne-moi son nom, même approximatif, ou dis-moi ce "
                    "qu'il fait et sur quelle taille d'emplacement.")
