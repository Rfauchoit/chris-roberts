...
def _alias_francais(con: sqlite3.Connection, *, echo=print) -> int:
    ...
    **Poids 1,0, comme le nom anglais.** Le mettre à 0,9 paraissait prudent —
    ...
    # **Ce que l'ingestion a refusé de nommer ne se renomme pas en français.**
    # `monde.py` écarte volontairement les 27 gisements `MineableRock_test_*`
    # et les 10 `_TEMPLATE` des alias — « ce qu'un joueur ne peut pas nommer
    # n'a pas à être nommable ». Cette passe-ci les réindexait sous leur nom
    # français, si bien qu'une entrée d'outillage n'existait plus que par
    # là : mesuré au rejeu du journal du 2026-08-21, « où miner de l'or »
    # demandait « tu parles duquel ? **Gold**, ou **Or** ? » — un choix entre
    # un minerai et sa propre traduction, le second étant en fait
    # `MineableRock_test_Gold`, invisible parce que privé de nom canonique.
    #
    # On ne recopie pas le motif `_test_` ici : deux listes écrites à la main
    # finissent par diverger, le projet l'a payé. Le critère est la
    # **décision déjà prise** — pas de nom canonique, pas de nom français.
    nommables = {(r[0], r[1]) for r in con.execute(
        "SELECT entity_type, entity_id FROM aliases WHERE source = 'canonical'")}
    lignes = []
    for r in con.execute("SELECT entity_type, uuid, nom_fr FROM traductions "
                         "WHERE nom_fr IS NOT NULL").fetchall():
        norme = normalize(r[2])
        if not norme or (r[0], r[1]) not in nommables:
            continue
