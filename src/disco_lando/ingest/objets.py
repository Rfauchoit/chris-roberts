        ...
        for entry in read_json(path):
            ...
            if composant:
                con.execute(
                    ...
                    "(item_uuid, shield_health, shield_regen, shield_damaged, "
                    " shield_downed, qt_jump_range, qt_drive_speed, qt_cooldown, "
                    " qt_spool_time, qt_fuel_rate, radar_aim_min, radar_aim_max, "
                    " radar_aim_buffer, "
                    " health, item_mass, cooling_rate, power_rate, "
                    " signature_em, signature_ir) "
                    "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    (uuid, composant["shield_health"], composant["shield_regen"],
                     composant["shield_damaged"], composant["shield_downed"],
                     composant["qt_jump_range"], composant["qt_drive_speed"],
                     composant["qt_cooldown"], composant["qt_spool_time"],
                     composant["qt_fuel_rate"], composant["radar_aim_min"],
                     composant["radar_aim_max"], composant["radar_aim_buffer"],
                     composant["health"],
            ...
            if stats:
                stats["reload"] = _duree_de_chargeur(
                    root, entry.get("className"))
                ...
                con.execute(
                    ...
                    " spread_attack, spread_decay, pierceability, "
                    " projectile_mass, max_penetration_thickness, "
                    " projectile_radius, penetration_base_distance, "
                    " penetration_near_radius, penetration_far_radius, "
                    " alpha_physical, alpha_energy, alpha_thermal, "
                    " alpha_biochemical, alpha_distortion, alpha_stun, "
                    " reload_seconds) "
                    "VALUES (" + ",".join("?" for _ in range(40)) + ")",
                    (uuid, classe, genre, stats["dps"], stats["alpha"], stats["rpm"],
                     ...
                     stats["spread_minimum"], stats["spread_maximum"],
                     stats["spread_first_attack"], stats["spread_attack"],
                     stats["spread_decay"], stats["pierceability"],
                     stats["projectile_mass"],
                     stats["max_penetration_thickness"],
                     stats["projectile_radius"],
                     stats["penetration_base_distance"],
                     stats["penetration_near_radius"],
                     stats["penetration_far_radius"],
...
#: Le dossier `items/` n'est **pas** ingéré — 11 045 fichiers de mobilier
#: et d'intérieur de vaisseau, mesurés en phase 1. On y lit pourtant les
#: armes, une par une : le champ cherché n'existe que là, et la population
#: est bornée à 828 fichiers.
_CACHE_DUREE: dict[str, float | None] = {}


def _duree_de_chargeur(root: pathlib.Path,
                       class_name: str | None) -> float | None:
    """`unstowMagDuration` — le seul temps de rechargement que CIG publie.

    **Le projet avait conclu qu'il n'existait pas.** C'était vrai du résumé
    `stdItem` que lit l'index, et faux du fichier d'objet : le champ vit
    dans `Raw.Entity.Components.SCItemWeaponComponentParams.ammoRepoolParams`.
    L'utilisateur a insisté — « il doit bien y avoir un temps de
    rechargement global communiqué par CIG quelque part » — et il avait
    raison. C'est le geste « la conclusion *la donnée n'existe pas* est
    celle qui coûte le plus cher », pour la troisième fois du projet après
    `refineries_yields` et le raffinage des journaux.

    Trois paliers sur 828 armes, et ils discriminent : 1 s pour les
    fusils, SMG, pistolets, fusils à pompe et snipers ; 1,5 s pour les
    armes d'épaule ; 3 s pour le lourd. Les armes de vaisseau valent 3
    uniformément — la colonne n'y sépare rien, et le rendu doit le savoir.

    Absent se rend `None` : un dépôt plus ancien, un fichier manquant ou un
    objet sans composant d'arme ne doivent pas faire échouer l'ingestion.
    """
    if not class_name:
        return None
    if class_name in _CACHE_DUREE:
        return _CACHE_DUREE[class_name]
    valeur = None
    chemin = root / "items" / f"{class_name}.json"
    try:
        if chemin.is_file():
            brut = json.loads(chemin.read_text(encoding="utf-8"))
            arme = ((brut.get("Raw") or {}).get("Entity", {})
                    .get("Components", {})
                    .get("SCItemWeaponComponentParams") or {})
            repool = arme.get("ammoRepoolParams")
            if isinstance(repool, dict):
                brute = repool.get("unstowMagDuration")
                valeur = float(brute) if brute not in (None, "") else None
    except (OSError, ValueError, TypeError, AttributeError):
        valeur = None
    _CACHE_DUREE[class_name] = valeur
    return valeur
