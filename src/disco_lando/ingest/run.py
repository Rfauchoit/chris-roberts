...
def ingest(source_dir: pathlib.Path | None = None,
    ...
    try:
        ...
        # Même raison que le soutenu : le délai décrit un générateur, mais il
        # faut le loadout du vaisseau pour savoir lequel est monté.
        counts["delais_bouclier"] = loaders.load_delais_bouclier(con, root)
        counts["composants_exposes"] = loaders.load_composants_exposes(
            con, root)
        # L'EMP et les pools de distorsion : après les items, puisqu'on
        # remonte des `class_name` déjà en base vers leurs fichiers.
        counts["emp"] = loaders.load_emp(con, root)
        counts["anomalies_duel"] = loaders.load_anomalies_duel(con)
        echo(f"items    {counts['items']:>7}   emp/distorsion {counts['emp']}")
...
def _schema_attendu() -> dict[str, frozenset[str]]:
    """Carte exacte ``table -> colonnes`` produite par ``schema.sql``.

    Une regex globale ne conservait pas le propriétaire d'une colonne :
    ``foo`` absente de A paraissait présente dès que B possédait aussi un
    ``foo``. Exécuter le vrai schéma dans SQLite couvre en plus les colonnes
    générées et les tables virtuelles sans maintenir un second parseur.
    """
    return db.schema_attendu()


def _colonnes_attendues() -> set[str]:
    """Compatibilité des contrôles historiques : union des colonnes."""
    return {colonne for colonnes in _schema_attendu().values()
...
def needs_reingest(db_path: pathlib.Path | None = None,
    ...
    try:
        ...
        presentes = {
            table: {r["name"] for r in con.execute(
    ...
    manquantes = [
        f"{table}.{colonne}"
        for table, colonnes in _schema_attendu().items()
        for colonne in sorted(colonnes - presentes.get(table, set()))
    ]
    if manquantes:
        apercu = ", ".join(manquantes[:4])
