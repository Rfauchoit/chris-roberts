...
async def lifespan(app: FastAPI):
    ...
    # **Cette connexion-ci ne sert qu'au démarrage et à l'arrêt.** Les
    # requêtes des joueurs passent par `_con()`, qui en ouvre une **par
    # thread** : le raisonnement d'origine — « lecture seule, aucun écrivain
    # concurrent, donc aucun verrou à craindre » — ne parlait que des
    # verrous d'écriture, et `check_same_thread=False` autorise en plus
    # plusieurs threads à se servir du même objet. Voir `_con()`.
...
def _identite_code() -> dict[str, Any]:
    ...
    # Lu **une fois au démarrage** (l'appelant est le `lifespan`) : le cœur
    # rapporte le code qu'il a chargé, pas celui du disque à l'instant t.
    # C'est toute la différence qui a permis de voir l'incident.
    return {"commit": commit_court(),
...
#: **Une connexion par thread de travail, pas une pour tout le monde.**
#: Le cœur partageait l'unique connexion ouverte au démarrage, avec
#: `check_same_thread=False` justifié par « aucun écrivain concurrent, donc
#: aucun verrou à craindre ». Le raisonnement ne couvrait que les **verrous
#: d'écriture** : le drapeau autorise aussi plusieurs threads à se servir du
#: *même objet* connexion, ce que `sqlite3` ne sérialise pas — le curseur et
#: l'état d'exécution sont partagés. FastAPI servant les endpoints
#: synchrones dans un pool, deux joueurs simultanés suffisaient.
#:
#: Mesuré le 2026-08-21, 50 questions à 8 en parallèle : **2 pannes** sur le
#: code du 2026-08-21 matin, **7** le soir — le défaut est ancien, et il
#: s'aggrave à chaque requête SQL ajoutée par question. Les deux symptômes
#: sont ceux d'un usage concurrent : `sqlite3.InterfaceError: bad parameter
#: or other API misuse`, et `PRAGMA data_version` qui rend `None`.
#:
#: Le joueur ne voyait pas un plantage mais « une panne de mon côté » — une
#: réponse fausse à une question juste, ce qui est le pire cas.
#:
#: La base du jeu est en lecture seule : une connexion par thread ne coûte
#: qu'un descripteur, et le pool de FastAPI est borné. La base de **guilde**
#: n'a jamais eu le problème — ses 27 routes ouvrent déjà leur connexion.
_par_thread = threading.local()


def _con() -> sqlite3.Connection:
    if _state.get("con") is None:
        raise HTTPException(503, "base indisponible")
    con = getattr(_par_thread, "con", None)
    if con is None:
        con = sqlite3.connect(f"file:{config.DB_PATH}?mode=ro", uri=True)
        con.row_factory = sqlite3.Row
        _par_thread.con = con
