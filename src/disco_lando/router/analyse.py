...
def _ngrams(question: str) -> list[str]:
    ...
    """
    # **Un jeton d'une lettre ne porte pas d'identite — sauf le numero d'une
    # marque.** `normalize` decoupe « Mk2 » en « mk 2 », et ce « 2 » se
    # faisait jeter ici : le gramme « aurora mk 2 » ne pouvait jamais se
    # former, seul « aurora mk » restait, et il sort *Aurora Mk I CL Ship
    # Armor* a 93 par l'etage `flat` quand « aurora mk 2 » sort le vaisseau a
    # 95. Mesure au rejeu du journal, 2026-08-21 : deux ecritures du meme
    # vaisseau rendaient deux autonomies differentes, sans qu'aucun signal ne
    # dise laquelle etait la bonne.
    # **Et la lettre d'une désignation de modèle est du même bois.** Trouvé
    # en auditant `rendu-et-conversation`, 2026-08-22 : « les composants du
    # **Anvil F7C-S Hornet Ghost Mk I** » — le nom officiel, écrit tel quel —
    # produisait le gramme « anvil f7c hornet ghost mk i ». Le **« s »** de
    # « F7C-**S** » était tombé, et le nom tapé ne pouvait donc jamais
    # correspondre à l'entité : le bot demandait « tu veux dire… ? » en
    # proposant trois Hornet, et recopier exactement sa proposition rendait
    # « je n'ai pas compris ». Deux défauts empilés pour une lettre.
    #
    # Le critère est le **voisin** : un jeton d'une lettre garde son sens
    # quand il prolonge une référence — « mk » ou un jeton qui porte un
    # chiffre (« f7c », « p6 »). Ailleurs, il reste du bruit : « j'ai »,
    # « c'est », « l'or » n'y gagnent rien.
    def _prolonge_une_reference(precedent: str) -> bool:
        return precedent == "mk" or any(c.isdigit() for c in precedent)

    # Et le contrôle passe **avant** `STOPWORDS` : « s » y figure — c'est
    # l'élision et le pluriel français — si bien que le « S » de « F7C-S »
    # se faisait jeter deux lignes trop tôt. Une lettre qui prolonge une
    # référence n'est plus un mot de la langue.
    bruts = _INTENT_WORDS.sub(" ", normalize(question)).split()
    words = []
    for indice, mot in enumerate(bruts):
        designation = (len(mot) <= 1 and indice
                       and _prolonge_une_reference(bruts[indice - 1]))
        if not designation and mot in STOPWORDS:
            continue
        if len(mot) <= 1 and not designation:
            continue
        words.append(mot)
    grams: list[str] = []
...
def _alias_exact(con: sqlite3.Connection, gram: str) -> bool:
    ...
    **Les deux écritures d'une marque comptent pour une.** « Combien coûte un
    aurora mk2 » perdait son « 2 » — un motif d'intention le lit comme un
    nombre — et `sans_les_mots_d_un_nom` ne le réhabilitait pas, « aurora mk
    2 » n'étant l'alias exact de rien : la base écrit « Aurora Mk **II** ».
    Le gramme retombait à « aurora mk » et le prix partait sur une armure de
    vaisseau. Mesuré au rejeu du journal, 2026-08-21.
    """
    from ..resolver import _autre_ecriture_de_marque

    norme = normalize(gram)
    formes = [norme]
    if (autre := _autre_ecriture_de_marque(norme)) is not None:
        formes.append(autre)
    return bool(con.execute(
        "SELECT 1 FROM aliases WHERE alias_norm IN "
        f"({','.join('?' * len(formes))}) LIMIT 1", formes).fetchone())
