...
def _serveur_local_repond() -> bool:
    """Y a-t-il quelqu'un sur le port du LLM local ?

    On ouvre une socket plutôt que d'envoyer la requête : le coût d'un refus
    de connexion ne dépend pas du timeout HTTP mais de la pile réseau, et
    c'est lui qu'il faut borner. `create_connection` avec un timeout court
    rend `TimeoutError` avant que Windows n'ait fini ses tentatives.
    """
    hote = urllib.parse.urlparse(config.LOCAL_LLM_URL)
    port = hote.port or (443 if hote.scheme == "https" else 80)
    try:
        socket.create_connection((hote.hostname or "127.0.0.1", port),
                                 timeout=config.LOCAL_LLM_SONDE).close()
        return True
    except OSError:
        return False
...
def route_local(con: sqlite3.Connection, question: str) -> ToolCall | None:
    ...
    """
    # **Un étage absent doit céder en millisecondes, pas en secondes.**
    # Mesuré le 2026-08-16 : sur Windows, se faire refuser une connexion sur
    # `127.0.0.1:8080` prend **2 045 ms** — le refus lui-même est lent, quel
    # que soit le timeout de la requête. La cascade appelle cet étage deux
    # fois, donc **4,1 s** par question non routée, sur une machine où
    # `llama-server` ne tourne plus depuis la décision du 2026-08-08.
    #
    # Le contrat « un étage en panne passe la main » était tenu ; son prix ne
    # l'était pas. Une sonde TCP à timeout court rend la main en 150 ms, et
    # elle se refait à chaque appel : le serveur peut être rallumé entre deux
    # questions, et mémoriser son absence ferait rater ce réveil.
    if not _serveur_local_repond():
        return None
