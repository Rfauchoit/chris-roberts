...
from .motifs import MOTIF_POSTE_DE_FABRICATION, MOTIF_QUOI_FAIRE
...
def _prep_missions_payantes(c: Candidature) -> bool:
    ...
    if not (lieux or difficulte
        ...
        return False
    # **Et une mission nommée fait taire le classement.** « Combien paye la
    # mission Secure Site » rendait « les missions les mieux payées : 711 750
    # aUEC — Direct Large Cargo Haul… » : exact, chiffré, et sur un contrat de
    # fret que la question ne mentionne pas. Le mot « paye » ouvrait la porte,
    # et rien ne la refermait quand la question désignait **une** mission.
    #
    # Le seuil est **95 et pas 90**, mesuré : « secure site » sort à 100 et
    # « the price of freedom » à 95, quand les faux positifs du classement
    # plafonnent en dessous — « stanton » rend un contrat à 90 (c'est un
    # système), « combat » en rend un à 93 (c'est une famille d'activité).
    # C'est l'écart qui sépare un titre de mission d'un mot que la question
    # emploie pour filtrer.
    nommee = extract_entity(c.con, c.question, ("contract",))
    if (nommee and nommee[1] >= 95.0
            and normalize(nommee[0]) != normalize(syst or "")
            and not trouve_activite):
        return False
...
@preparateur("fabricateur")
def _prep_fabricateur(c: Candidature) -> bool:
    # **Le garde-fou de l'outil : le vocabulaire de l'emplacement.** Sans l'un
    # de ces mots, « les stats d'un P6-LR 900 » est la question de
    # `fiche_qualite` et doit le rester — le fabricateur ne prend que les
    # questions qui demandent **où** charger, jamais celles qui ne parlent que
    # de qualité. Le vocabulaire se lit dans `motifs`, il ne s'y recopie pas.
    if not MOTIF_POSTE_DE_FABRICATION.search(normalize(c.question)):
        return False
    # Deux qualités nomment une **comparaison**, pas un chargement : on laisse
    # `comparer_qualites` prendre plutôt que d'en choisir une au hasard.
    ...
    # **Le matériau qu'on tient n'est pas l'objet qu'on fabrique.** « Où je
    # mets mon Stileron 900 dans un P6-LR » nomme deux entités, et le
    # résolveur peut très bien préférer le minerai — la réponse serait alors
    # « Stileron ne se fabrique pas », exacte et à côté. Tout objet
    # fabricable porte un alias de type `blueprint` (l'ingestion l'ajoute
    # depuis le nom de sortie) : chercher ce type retrouve la cible sans
    # jamais avoir à reconnaître un minerai.
    cible = extract_entity(c.con, c.question, ("blueprint",),
                           exclus=_mots_d_intention(None, c.question))
    if cible is not None:
        c.args["query"] = cible[0]
        c.gram, c.entity_score = cible
...
def _prep_item_stats(c: Candidature) -> bool:
    # **« Dans une soute » est une question de capacité, pas de fiche.**
    # « Combien de balles dans une soute de Cutlass Black » rendait
    # *Cutlass Black Ship Armor — aucune statistique en base* : la fiche d'un
    # blindage, sur une question de chargement. `combien_dans_la_soute`
    # gagne pourtant l'intention (8,5 contre 3,5) et abandonne à juste titre
    # — « balles » ne nomme aucun objet rangeable — mais rien ne refermait
    # la porte derrière lui. Mesuré le 2026-08-21.
    #
    # Se taire vaut mieux : le repli propose alors ce qu'il sait dire du
    # vaisseau nommé, et le joueur reformule avec l'objet qu'il veut charger.
    if re.search(r"\bdans (?:la|une|ma|sa|le|mon|leur) (?:soute|cale|cargo)\b",
                 normalize(c.question)):
        return False
...
#:
#: **Il se lit dans `motifs.MOTIF_QUOI_FAIRE`, il ne s'y recopie plus.** Les
#: deux listes avaient divergé : élargir le motif à « qu'est-ce que **je**
#: peux faire » faisait scorer l'intention sans que ce garde-fou l'accepte,
#: et la question restait muette. Ce qui reste écrit ici est ce que l'écart
#: protégeait — les mots qui suffisent à **accepter** sans suffire à
#: **déclencher**.
_MOTS_D_ACTIVITE = re.compile(
    MOTIF_QUOI_FAIRE
    + r"|\bactivites?\b|\bs (?:amuser|occuper)\b"
...
_MOTS_DE_DEBUT = re.compile(
    ...
    # Les tournures à l'infinitif et le vocabulaire du store — mesurées le
    # 2026-08-19 contre le corpus citizen-starter-guide.com : « quelles
    # missions pour débuter » et « quel starter » restaient muets, le
    # motif côté routeur marquait mais ce garde-fou refusait. C'est le
    # couple déclencheur/garde-fou de l'US-3 : les deux bougent ensemble.
    # Le « | » ouvre la ligne : la sentinelle des motifs partagés compare
    # les littéraux **entiers**, et le coller à la ligne du dessus l'a fait
    # crier « paire plus observée » — à raison.
    r"|\bpour (?:debuter|commencer|demarrer)\s*$"
    r"|\bpremieres? missions?\b|\bstarters?\b"
)

#: Les déclencheurs qui suffisent **même quand la phrase nomme une entité** :
#: « quel starter », « les premières missions », « je débute avec un
#: Aurora ». Le suffixe « pour débuter », lui, n'y est pas — voir le
#: préparateur.
_MOTS_DE_DEBUT_FORTS = re.compile(
    r"\bpar (?:ou|quoi) (?:je |on |tu |)(?:commenc|demarr)\w*|"
    r"\b(?:je|on) (?:debute|commence)\b|\bdebutant\w*\b|"
    r"\bnouveau\w* joueur\w*\b|\bje viens de (?:commencer|debuter)\b|"
    r"\bpremiere fois que je joue\b|\bjamais joue\b|"
...
def _prep_par_ou_commencer(c: Candidature) -> bool:
    norm = normalize(c.question)
    if not _MOTS_DE_DEBUT.search(norm):
        return False
    # **Le suffixe « pour débuter » n'est un déclencheur que seul.** Quand la
    # phrase porte déjà une entité — « combien coûte un Aurora pour
    # débuter », « je peux aller de Crusader à Levski pour commencer » —
    # c'est un qualificatif, et le 4,5 du motif écrasait le prix (3,5) et le
    # trajet (4,0). Mesuré le 2026-08-19. Si seul le suffixe a ouvert la
    # porte, on abandonne dès qu'une entité forte se résout hors des mots
    # d'intention : l'intention suivante — la bonne — reprend la main.
    if not _MOTS_DE_DEBUT_FORTS.search(norm) and extract_entity(
            c.con, c.question,
            ("ship", "item", "starmap", "contract", "resource")) is not None:
        return False
