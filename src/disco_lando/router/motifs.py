...
from ..stats import CLASSES_D_ARME, GENRES_D_ARME
from ..equipement import _FAMILLES as _FAMILLES_D_EQUIPEMENT

# **Une seule source pour le vocabulaire des familles d'objets.** Le motif
# de `catalogue_objets` recopiait la liste à la main et avait déjà divergé
# du vocabulaire fermé qu'il est censé refléter. Il s'en déduit désormais :
# déclarer une famille dans `_FAMILLES_D_OBJETS` suffit à la router.
_MOTIF_FAMILLES_D_OBJETS = "|".join(
    motif for _libelle, motif, _clause, _mode in _FAMILLES_D_OBJETS)

# **La même famille, mais suivie d'un seuil chiffré.** Donner le motif nu
# aux deux outils créait mécaniquement une frontière fragile : ils
# marquaient sur exactement les mêmes mots, et le cliquet du banc l'a vu.
# Ce motif-ci ne se déclenche que sur « quels répéteurs font **plus de
# 500** dps » — jamais sur « c'est quoi les répéteurs », qui appartient au
# catalogue seul. Les tournures de seuil sont celles de `stats._SEUIL`.
#
# **Les quantificateurs sont non gloutons, et c'est indispensable.**
# `_mots_d_intention` retire de la recherche d'entité *tout* le texte
# apparié, sur l'union des motifs de tous les outils : un `.{0,40}` glouton
# avalait « quels répéteurs de plus de 500 dps pour un **gladius** plus de
# 3 » jusqu'au second seuil, emportant le vaisseau avec lui. C'est la règle
# « un motif d'intention ne doit pas enjamber l'entité », et elle se paie
# pour tous les outils, pas seulement celui qui écrit le motif.
# **Même leçon, deuxième endroit.** Le motif de `classer_equipement`
# recopiait lui aussi ses familles à la main, et il avait divergé de la même
# façon : les composants de vaisseau ajoutés à `equipement._FAMILLES` le
# 2026-08-14 étaient détectés et jamais routés, si bien que « quel est le
# meilleur bouclier taille 2 » restait muet. Une famille se déclare à un
# seul endroit, ici comme pour le catalogue.
_MOTIF_FAMILLES_D_EQUIPEMENT = "|".join(
    motif for _nom, motif, _types in _FAMILLES_D_EQUIPEMENT)

#: **Le genre d'arme est une famille comme une autre au moment d'un seuil.**
#: `_weapon_filter` sait lire « canon balistique » depuis toujours, et le
#: routeur l'ignorait : « quels canons balistiques font plus de 300 dps »
#: plafonnait à 2,0 chez `objets_au_seuil` — sous le seuil de confiance — et
#: ne routait nulle part. Le vocabulaire se lit dans `stats`, il ne se
#: recopie pas.
#:
#: Le seuil chiffré est ce qui rend l'ajout sûr : « quel canon monter sur un
#: Gladius » n'en porte pas et reste à `get_compatible_items`.
_MOTIF_GENRES_D_ARME = "|".join(
    sorted({rf"\b{mot}s?\b" for mot, _ in (*CLASSES_D_ARME, *GENRES_D_ARME)}))

#: **« Quoi faire » — une seule écriture, lue par le motif et par le
#: garde-fou.** `preparateurs._MOTS_D_ACTIVITE` en était une **seconde copie
#: à la main**, et les deux avaient divergé : le 2026-08-21, élargir le motif
#: à « qu'est-ce que **je** peux faire » a fait scorer l'intention sans que
#: le garde-fou l'accepte, donc la question restait muette. Exactement la
#: leçon déjà payée sur `catalogue_objets` et `classer_equipement`.
#:
#: Ce qui a motivé l'élargissement : « qu'est-ce que je peux faire dans le
#: jeu » rendait *Breakfast Dog* — le gramme « jeu » matchant « Dog Petit
#: Dé**jeu**ner » par le filet fuzzy. La fausse réponse masquait le trou du
#: motif ; c'est en retirant l'une qu'on a vu l'autre.
#:
#: **Ce que l'écart protégeait**, et qui est conservé : le garde-fou
#: acceptait `\bactivites?\b` seul et « s'amuser / s'occuper », que le motif
#: n'a jamais portés. Ils restent au garde-fou, qui est le plus large des
#: deux — un mot qui suffit à *accepter* ne suffit pas à *déclencher*.
MOTIF_QUOI_FAIRE = (
    r"\bquoi faire\b|\bque faire\b|\bon fait quoi\b|\bque fait ?on\b|"
    r"\bqu est ce (?:qu on|que je|que tu|qu il|qu elle|que nous)\s+"
    r"(?:peu[txt]|peuvent|pouvons|fai[ts]|faisons|pourrai\w*)\b|"
    r"\bquoi jouer\b|\bon joue a quoi\b")

#: Les pièces d'une recette, telles que le joueur les nomme — écrites **une
#: seule fois**. Le motif de déclenchement et le garde-fou du préparateur s'en
#: déduisent tous les deux : un garde-fou recopié à côté de son motif diverge,
#: et la divergence rend muet sans que rien ne le signale (mesuré sur
#: `_MOTS_D_ACTIVITE`, deux jours de questions sans réponse).
_PIECES_DE_RECETTE = r"emplacements?|composants?|slots?|postes?|pieces?"

#: Tout le vocabulaire du poste de fabrication. Le préparateur l'**exige** ;
#: les motifs, eux, n'en demandent que des formes plus contraintes — « dans
#: quel emplacement », « où mettre … qualité ». C'est l'asymétrie habituelle
#: du couple déclencheur/garde-fou : au moment du garde-fou, l'intention est
#: déjà reconnue, et un mot qui suffit à *accepter* ne suffit pas à
#: *déclencher*.
MOTIF_POSTE_DE_FABRICATION = re.compile(
    rf"\bfabricateurs?\b|\bitem ?fab\w*|\b(?:{_PIECES_DE_RECETTE})\b|"
    r"\bsimule\w*\b|\brepart\w*|"
    r"\bou (?:je |on |tu )?(?:mets?|mettre|place\w*|charge\w*)\b")

_MOTIF_FAMILLE_AU_SEUIL = (
    r"(?:" + _MOTIF_FAMILLES_D_OBJETS + r"|" + _MOTIF_GENRES_D_ARME + r").{0,25}?"
    r"\b(?:plus|moins|au moins|au plus|superieur\w*|inferieur\w*"
    r"|en dessous|sous|au dessus|dessus)\b.{0,12}?\d")
...
_INTENTS: dict[str, list[tuple[str, float]]] = {
    ...
    "get_item_stats": [
        ...
        # **La moitié manquante de « combien de balles dedans ».** CIG
        # publie la durée de manipulation du chargeur ; on peut donc
        # répondre « combien de temps pour recharger ». Le mot s'écrit au
        # long — `recharge` nu appartient au **bouclier**, chez
        # `classer_equipement`, et la forme verbale ne le touche pas.
        (r"\brechargement\b|\brecharger\b", 3.0),
        (r"\bcapacitor\w*|\bcapaciteur\w*|\bcondensateur\w*", 3.5),
        (r"\bcadence\b|\brpm\b", 2.5),
        # **2,0 et pas plus, mesuré le 2026-08-14.** Ces mots valent moins
        # ici que chez `chaine_de_qualites` (3,0), si bien que « les stats
        # du P6-LR » gagne par repli à 0,76 — dans la bande de réserve. Les
        # monter à 3,0 semblait réparer ça ; c'était passer devant
        # `get_ship_stats`, et « les caractéristiques du Gladius » rendait
        # alors *Gladius Model*, le bibelot de bureau. Le doute est le bon
        # comportement : trois outils se disputent légitimement « stats »,
        # et la réserve de confiance existe pour ce cas.
    ...
    "classer_equipement": [
        ...
        # Le vocabulaire fermé de `equipement._FAMILLES`, déduit et non
        # recopié. Il porte l'équipement personnel, les missiles, et les
        # composants de vaisseau — y compris ceux que l'outil ne classe
        # pas mais sait **expliquer** : 6 noms distincts de propulseur dans
        # tout le catalogue, aucune caractéristique propre aux 77 radars.
        # Se taire laisserait croire à une lacune.
        (_MOTIF_FAMILLES_D_EQUIPEMENT, 3.5),
    ...
    # plus par accident.
    # **Le poste de fabrication.** Le fabricateur du jeu charge un matériau
    # par emplacement ; la fiche de qualité, elle, applique une qualité unique
    # à tout l'objet. « Dans quel composant je mets mon Stileron 900 » n'avait
    # donc aucun outil : la question tombait chez `fiche_qualite` (5,5 sur
    # « qualité »), qui répond juste et à côté — un multiplicateur global là
    # où le joueur demande **où** charger son minerai.
    #
    # Les poids passent au-dessus de 5,5 sur le vocabulaire propre à la
    # machine, et le préparateur exige ce vocabulaire : sans lui, l'outil
    # volerait toutes les questions de qualité. Les tours qui nomment un
    # matériau utilisent un **lookahead**, qui ne consomme rien — un motif
    # qui enjambe l'entité l'efface pour tout le monde.
    "fabricateur": [
        (r"\bfabricateurs?\b|\bitem ?fab(?:ricator)?s?\b", 7.0),
        (r"\bemplacements? de (?:fabrication|craft\w*)\b|"
         r"\bpostes? de fabrication\b", 7.0),
        # « Je mets quoi dans **chaque** emplacement » est la formulation la
        # plus naturelle du poste, et elle partait chez `get_blueprint` (3,0
        # sur « fabrique ») : la recette, sans dire où va quoi.
        (rf"\bdans (?:chaque|quel(?:le)?s?) (?:{_PIECES_DE_RECETTE})\b", 6.5),
        (r"\bsimule\w*\b.{0,14}?\b(?:fabrication|craft\w*)\b", 6.5),
        (rf"\bquel(?:le)?s? (?:{_PIECES_DE_RECETTE})\b"
         r"(?=.{0,25}\b(?:qualite|materia|minerai|fabriqu|craft))", 6.5),
        # À 6,0, la question réelle « où mettre mon minerai de qualité 900 »
        # restait à exactement 0,5 point de `fiche_qualite` : 20 frontières
        # fragiles sur 530 questions, et le test de croissance montait à
        # 4,07 %. La marge pleine ramène la mesure à 19 sans changer aucun
        # des 301 routages étiquetés ; le préparateur garde le périmètre.
        (r"\bou (?:je |on |tu )?(?:mets?|mettre|place\w*|charge\w*)\b"
         r"(?=.{0,25}\b(?:qualite|materia|minerai|meilleur))", 6.5),
        (r"\brepart(?:ir|ition|is)\b"
         r"(?=.{0,25}\b(?:qualite|materia|minerai))", 6.5),
        (r"\bquel(?:le)?s? (?:materiaux?|minerais?)\b.{0,12}?\b(?:dans|pour)\b",
         5.5),
    ],
    "fiche_qualite": [
    ...
    # répondait armement.
    # « En quoi je peux upgrade mon Polaris » — question du corpus restee
    # sans aucun outil. Le vocabulaire d'amelioration passe devant celui
    # des composants, qui repond « ce qui est monte » et non « ce qu'on
    # pourrait monter ».
    "marges_d_upgrade": [
        (r"\bupgrade\w*\b|\bameliorer\b|\bamelioration\b", 5.0),
        (r"\ben quoi je peux\b", 5.5),
        (r"\bquoi changer\b|\bque changer\b", 5.0),
        (r"\bmarges? de progression\b", 5.5),
    ],
    "get_ship_components": [
    ...
    "objets_au_seuil": [
        ...
        # Le même vocabulaire de familles que le catalogue : « quels
        # répéteurs font plus de 500 DPS » nomme une famille **et** un
        # seuil. Sans cette ligne l'intention n'existait pour personne, et
        # le « 500 » partait résoudre le générateur *JS-500* — une fiche
        # exacte, sur un objet que la question ne mentionne pas. Le poids
        # vaut celui de « quelles armes » : c'est la même tournure, avec un
        # nom de famille à la place du mot générique. Le motif porte le
        # seuil avec lui : sans ça, les deux outils marquaient sur les
        # mêmes mots et se touchaient sur les catalogues nus.
        (_MOTIF_FAMILLE_AU_SEUIL, 3.0),
    ],
    ...
    "combien_y_a_t_il": [
        ...
        # **« Quels systèmes » est la même question que « combien ».** Rejeu
        # du journal, 2026-08-21 : « quelles sont les autres étoiles à part
        # Pyro » ne trouvait rien, alors que l'outil compte déjà les systèmes
        # et sait désormais les nommer. Le mot de famille est **exigé** —
        # c'est la règle « un outil sans entité exige son vocabulaire » — et
        # il est ici le plus étroit du catalogue : seuls le système et
        # l'étoile se comptent en petit nombre, donc se listent.
        (r"\b(?:quel(?:le)?s?|liste|cite|donne)\b.{0,25}?"
         r"\b(?:systemes?|etoiles?)\b", 3.0),
    ],
    ...
    # « Sur 10 combats, combien de fois gagne le Arrow ? » — demandée deux
    # fois au journal (2026-08-12 et 2026-08-13) et refusée les deux fois
    # faute de modèle. Les motifs sont **plus lourds que ceux du matchup**
    # (7,0) parce qu'ils le contiennent : la question porte « qui gagne »
    # *et* un compte de combats, et c'est le compte qui la distingue. Sans
    # ce surpoids, l'arbitre départagerait deux outils dont l'un ne répond
    # qu'à la moitié de la demande.
    "simuler_duel": [
        (r"\bsur \d+ (?:combats?|duels?|affrontements?|matchs?)\b", 9.0),
        (r"\b\d+ (?:combats?|duels?|affrontements?)\b", 8.5),
        (r"\bcombien de fois\b", 8.0),
        (r"\bsimul\w+", 8.0),
        # « Il gagne combien de fois sur dix » : le compte peut s'écrire en
        # lettres, et c'est la forme parlée la plus courante.
        (r"\bfois sur (?:dix|vingt|cent|\d+)\b", 8.5),
        (r"\bstatistiquement\b|\ben moyenne qui gagne\b", 7.5),
    ],
    ...
    "vaisseaux_par_metier": [
        ...
        # Le vaisseau de départ : « quel starter », « quel vaisseau pour
        # commencer ». 5,0 pour battre le 4,5 de `par_ou_commencer`, dont le
        # suffixe « pour commencer » marque aussi — la question nomme un
        # **vaisseau**, c'est lui le sujet, pas le parcours du débutant.
        (r"\bstarters?\b", 5.0),
        (r"\bvaisseaux?\b.{0,25}?\bpour (?:debuter|commencer|demarrer)\b", 5.0),
    ],
    "comment_gagner": [
    ...
    "get_price": [
        ...
        # **« Acheter » n'est jamais un gisement.** Frontière la plus
        # fragile du banc : « où en acheter » sortait get_price et
        # where_to_find_resource **à égalité parfaite** (3,0 contre 3,0),
        # et seul l'arbitre les départageait — un motif ajouté à côté
        # pouvait retourner six questions d'un coup. Le verbe tranche :
        # on mine du quantainium, on l'achète à un terminal, et dans les
        # deux cas c'est le prix qu'on demande.
        (r"\bou (?:je peux |on peut |en |l |le |la |les )*achet\w*", 3.5),
    ...
    "progression_dans": [
        ...
        # **« Comment avoir la réputation Shubin »** — la formulation de
        # l'utilisateur le 2026-08-20, et elle partait chez
        # `get_mission_reputation`, qui parle d'**une** mission. « Monter »
        # était couvert, « avoir », « obtenir », « débloquer » non : c'est
        # la même question posée avec le verbe qui vient d'abord.
        #
        # **Le poids est mesuré, pas parié** : `get_mission_reputation`
        # sort à 5,5 sur cette question — il empile « reputation » (3,0)
        # et son abréviation (2,5) — quand ces motifs plafonnaient à 4,5.
        # « Avoir la réputation » est strictement plus spécifique que
        # « réputation », et un mot d'intention plus spécifique doit peser
        # plus lourd que le générique qui l'englobe.
        (r"\b(?:monter|avoir|obtenir|gagner|debloquer|farmer) "
         r"(?:en |de |la |ma |du |le )?(?:reputation|rang|standing)\w*", 6.0),
    ...
    # vaisseau.
    "catalogue_objets": [
        # **Le motif se construit depuis le vocabulaire fermé**, il ne le
        # recopie plus. La liste écrite à la main avait déjà divergé : elle
        # ignorait « canons de vaisseau », que `_FAMILLES_D_OBJETS` accepte
        # depuis toujours — la famille était détectée, l'intention jamais,
        # et la question restait sans réponse. Une famille neuve se déclare
        # désormais à un seul endroit.
        (_MOTIF_FAMILLES_D_OBJETS, 4.0),
    ...
    "decrire": [
        ...
        # **« Où prendre la mission X ».** Trouvé par le banc adversarial
        # du 2026-08-16 : la question partait chez
        # `where_to_find_resource`, qui cherchait une *ressource* dans le
        # titre du contrat — « cargo », « small » suffisent à une
        # résolution faible. Une réponse fausse, pas un silence.
        #
        # La fiche répond déjà : elle porte « Où la prendre » avec ses
        # terminaux depuis le sprint qui l'a ouverte. Il ne manquait que
        # la tournure.
        (r"\bou (?:prendre|trouver|accepter|recuperer)\b.{0,12}?"
         r"\b(?:la mission|le contrat)\b", 5.0),
    ...
    # **« On fait quoi ce soir ? »** — la question du sprint 38. C'est un
    # outil sans entité, donc son préparateur exige son vocabulaire : sans
    # cette discipline il avalerait « quoi » et « faire », qui sont dans une
    # question sur deux. Les motifs restent donc des **locutions**, jamais
    # des mots isolés.
    "activites_a_faire": [
        (r"\b(?:on|je|nous) fait quoi\b|\bque fait ?on\b", 4.0),
        (MOTIF_QUOI_FAIRE, 4.0),
        (r"\bquelles? activites?\b|\bidees? d activites?\b", 4.0),
        (r"\bquoi jouer\b|\bon joue a quoi\b", 3.5),
        (r"\bactivites? (?:a faire|possibles?|dispo\w*)\b", 3.5),
        (r"\bpropose\w* (?:moi )?(?:une|des) activites?\b", 4.0),
        (r"\bce soir\b|\bcette session\b", 1.0),
    ],
    # « Par quoi je commence ? » — le parcours du débutant. Il partage la
    # famille d'`activites_a_faire` sans partager un seul motif : « quoi
    # faire » et « par où commencer » sont deux questions. Les poids sont à
    # 4,0 et 4,5 parce que l'outil n'a pas d'entité — `route()` exige alors
    # une intention franche —, et 4,5 va aux locutions qui ne peuvent rien
    # dire d'autre : « je débute » ne parle jamais d'un vaisseau.
    "par_ou_commencer": [
        (r"\bpar (?:ou|quoi) (?:je |on |tu |)commenc\w*", 4.5),
        (r"\bje (?:debute|commence)\b|\bon (?:debute|commence)\b", 4.5),
        (r"\bnouveau\w* joueur\w*\b|\bdebutant\w*\b", 4.5),
        (r"\bje viens de (?:commencer|debuter|acheter le jeu)\b", 4.5),
        (r"\bpremiere fois que je joue\b|\bjamais joue\b", 4.0),
        (r"\bquand on debute\b|\bpour bien demarrer\b", 4.0),
        (r"\bpar ou (?:je |on |)demarre\w*", 4.5),
        # **Le guide du démarrage était invisible sous ses tournures
        # naturelles.** Mesuré le 2026-08-19 contre le corpus de
        # citizen-starter-guide.com : « quelles missions pour débuter »
        # restait muet, et « quel vaisseau pour commencer » partait chez
        # `compare_ships` qui répondait la **vitesse max** — une réponse à
        # côté qui a l'air complète, le pire cas. L'infinitif après « pour »
        # est la forme la plus tapée, et aucun motif ne la lisait : tous
        # exigeaient un sujet conjugué (« je débute ») ou la locution
        # exacte (« par où commencer »). Le motif exige la **fin de
        # phrase** : « pour commencer les CZ » ou « pour débuter le
        # minage » nomment un sujet, et c'est lui qui doit router.
        # **5,0 et non 4,5, et le demi-point est mesuré.** À 4,5, « quelles
        # missions pour débuter » ne battait `missions_du_site` (4,0 sur
        # « quelles missions ») que d'un demi-point pile : routage juste,
        # frontière fragile au sens du banc — une décision prise sur rien.
        # À 5,0 l'écart vaut 1,0, le choix est délibéré.
        (r"\bpour (?:debuter|commencer|demarrer)\s*$", 5.0),
        (r"\bpremieres? missions?\b", 5.0),
    ],
    # « Quoi de neuf ? » — un outil sans entité de plus, donc des locutions
    # exigées et jamais un mot isolé : « neuf », « nouveau » et « patch »
    # traînent partout. Le préparateur exige en plus le vocabulaire.
    "quoi_de_neuf": [
        (r"\bquoi de (?:neuf|nouveau)\b", 4.0),
        (r"\bqu est ce qui (?:arrive|change|sort)\b", 4.0),
        (r"\bnouveaute?s?\b.{0,12}\bpatch\b|\bpatch\b.{0,12}\bnouveaute", 3.5),
        (r"\bprochaine? (?:version|patch|mise a jour)\b", 3.5),
        (r"\bnotes? de (?:version|patch)\b", 3.5),
        (r"\bil y a quoi dans la \d", 4.0),
    ],
    # La fiche d'une activité nommée — « raconte-moi Contested Zones »,
    # « c'est quoi le Storm Breaker », « comment on fait les Onyx ».
    # **Les poids sont au-dessus de 3,0, et ce n'est pas de l'inflation.**
    # `route()` exige une intention franche (≥ 3,0) pour tout outil sans
    # entité, « faute de second garde-fou ». Celui-ci en a pourtant un, et
    # strict : son préparateur abandonne si aucun des alias du catalogue —
    # neuf entrées — ne se retrouve dans la phrase. « Raconte-moi le
    # Gladius » n'y passe donc pas. On respecte le seuil du moteur plutôt
    # que de l'assouplir pour un cas particulier.
    "fiche_activite": [
        (r"\braconte\w*\b", 3.5),
        # **4,5, et le demi-point de plus est mesuré, pas prudentiel.**
        # « Comment on fait les onyx » rendait « *Strata Helmet Onyx* se
        # fabrique en 3 minutes, avec 0,03 SCU de Corundum » — un casque, sur
        # une question qui parle d'une chaîne de missions. Les deux outils
        # sortaient à **3,5 chacun**, et l'arbitrage donne alors la main au
        # plus contraint, `get_blueprint`.
        #
        # À 4,0 le routage devenait juste mais la frontière restait
        # *fragile* au sens du banc — un demi-point d'écart, c'est-à-dire une
        # décision prise sur rien. À 4,5 l'écart vaut 1,0 : le choix est
        # délibéré. Le plafond de confiance étant à 4,0 dans `route()`, ce
        # demi-point n'achète aucune assurance supplémentaire — il ne fait
        # qu'ordonner.
        #
        # Passer devant ne casse pas la fabrication, parce que c'est le
        # **préparateur** qui tranche pour de bon : « comment on fait le
        # P8-AR » ne résout aucun alias d'activité, l'intention est
        # abandonnée et la recette reprend la main. Mesuré dans les deux
        # sens au cahier de non-régression.
        (r"\bcomment (?:on |ca |)(?:se |)(?:fait|passe|joue|deroule)\b", 4.5),
        (r"\ben quoi (?:ca |cela |)consiste\b", 3.5),
        (r"\bexplique\w* (?:moi )?(?:le|la|les|l )\b", 3.0),
        (r"\bdetails? de\b|\bfiche de\b", 3.0),
        (r"\bc est quoi\b|\bqu est ce que\b", 1.0),
    ],
}
