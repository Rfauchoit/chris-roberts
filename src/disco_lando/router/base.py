...
TOOLS: dict[str, Tool] = {
    ...
    # **Sprint 38 — la question de la soirée.** Sans entité : c'est le
    # préparateur qui lit le nombre de joueurs et les envies, et qui exige
    # son vocabulaire. `joueurs` vaut None quand la phrase ne le dit pas —
    # le rendu **demande** alors, il ne suppose jamais.
    "activites_a_faire": Tool(
        "activites_a_faire", queries.activites_a_faire, "query", (),
        "Quoi faire dans le jeu ce soir, selon le nombre de joueurs, "
        "l'envie de combat, de mission ou de PvP, et le temps disponible.",
    ),
    # « Par quoi je commence ? » — le parcours d'un debutant. Outil a part et
    # non un argument de `activites_a_faire` : il ecarte les boucles, donc il
    # ne liste pas la meme population.
    "par_ou_commencer": Tool(
        "par_ou_commencer", queries.par_ou_commencer, "query", (),
        "Par quoi commencer quand on debute : les activites et chaines de "
        "missions dans l'ordre ou un nouveau joueur peut les aborder.",
    ),
    "fiche_activite": Tool(
        "fiche_activite", queries.fiche_activite, "query", (),
        "Le detail d'une activite : deroule, materiel a emporter, "
        "recompenses, contrats du jeu qui s'y rattachent.",
    ),
    # « Quoi de neuf ? » — la note du prochain patch. Chaque ligne dit si
    # elle est une annonce ou si l'objet est deja dans les fichiers du build
    # installe : c'est la seule chose qu'un guide ne sait pas dire.
    "quoi_de_neuf": Tool(
        "quoi_de_neuf", queries.quoi_de_neuf, "query", (),
        "Ce qui arrive au prochain patch du jeu, et ce qui en est deja "
        "verifiable dans les fichiers installes.",
    ),
    ...
    "simuler_duel": Tool(
        "simuler_duel", queries.simuler_duel, "query", ("ship",),
        "« Sur 10 combats, combien en gagne le Arrow ? » — la simulation "
        "d'un duel répété : distance choisie par chacun et tranchée par la "
        "vitesse, taux de touche tiré du temps de vol des projectiles, de "
        "l'agilité et de la taille, fuite possible quand on ne peut pas "
        "gagner. Modèle maison par-dessus les verrous publiés.",
    ),
    "bataille": Tool(
    ...
    "marges_d_upgrade": Tool(
        "marges_d_upgrade", queries.marges_d_upgrade, "query", ("ship",),
        "Ce qu'on peut ameliorer sur un vaisseau : les composants montes "
        "dont il existe un meilleur grade a la meme taille, et lesquels "
        "monter a la place.",
    ),
    ...
    "fabricateur": Tool(
        # La fiche de qualité applique **une** qualité à tout l'objet ; le
        # fabricateur du jeu charge un matériau par emplacement, et chaque
        # emplacement pilote ses propres statistiques. « Mon Stileron 900, je
        # le mets où ? » n'avait aucun outil.
        "fabricateur", queries.fabricateur, "query", ("blueprint", "item"),
        "Le poste de fabrication d'un objet : les emplacements de sa recette, "
        "le matériau que chacun accepte et sa qualité minimale, ce que la "
        "qualité de chaque emplacement fait varier, et où placer le meilleur "
        "matériau.",
    ),
    ...
    "armes_qui_marchent": Tool(
        "armes_qui_marchent", queries.armes_qui_marchent, "query", (),
        "Armes qui ont reellement tue chez les membres, d apres le "
        "Game.log — et contre quel type de cible. Observation, pas bareme.",
    ),
    "jamais_essaye": Tool(
        "jamais_essaye", queries.jamais_essaye, "query", (),
        "Activites du catalogue dont le carnet ne garde aucune trace chez "
        "les membres — en distinguant ce qu on sait voir de ce qu on ne "
        "sait pas voir.",
    ),
