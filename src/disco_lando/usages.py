...
def debit_depasse(instance: str) -> bool:
    """Une instance ne peut pas noyer la route ouverte."""
    maintenant = time.monotonic()
    with _VERROU:
        recents = [t for t in _debits.get(instance, [])
                   if maintenant - t < FENETRE_DEBIT]
        recents.append(maintenant)
        _debits[instance] = recents
        return len(recents) > ENVOIS_MAX_PAR_FENETRE
