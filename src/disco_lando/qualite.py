...
- **Deux composants peuvent porter la même statistique sans s'accorder.** Le
  ...
  **différentes**. Les multiplier entre eux fabriquerait un nombre que le jeu
  ne publie pas : on rend donc l'effet **par composant**.
...
def _blueprint(con: sqlite3.Connection, terme: str, question: str = "",
    ...
    for c in fenetre:
        ...
        if uuid is None:
            ...
            raise NotFound(f"je n'ai pas de recette pour « {terme} »")
...
def _qualite_requise(effets: tuple[Effet, ...], borne: float,
    ...
    effets se multiplient entre eux, chacun avec ses propres bornes, et le
    produit n'a pas d'inverse analytique simple. Un point sur 1 000 est très
    ...
    for pas in range(0, int(borne) + 1):
        produit = 1.0
        for e in chiffrables:
            produit *= e.multiplicateur(float(pas))
        if produit >= seuil - 1e-9:
            return float(pas)
    return None
...
def _mult_a(effets: tuple[Effet, ...], qualite: float) -> float:
    ...
    for e in effets:
        if e.chiffrable:
            produit *= e.multiplicateur(qualite)
    return produit
...
def qualite_pour_tuer(con: sqlite3.Connection, query: str, *,
    ...
    Le cas « impossible » est une réponse, pas un échec : le P6-LR plafonne à
    1,1 de multiplicateur, il lui manque 1 % de dégâts contre un casque lourd,
    et aucune qualité ne le lui donnera. Répondre 1000 serait faux ; répondre
    « je ne sais pas » le serait tout autant.
    """
    ...
    if not alpha:
        raise NotFound(f"je n'ai pas les dégâts par tir de {nom}")
    ...
    return {"nom": nom, "item_uuid": item_uuid, "alpha": alpha, "plombs": plombs,
            ...
            # **Le jeu ne dit pas comment deux composants se combinent.** Le
            # P6-LR a un canon et des pièces de précision, chacun de 0,9 à 1,1
            # sur les dégâts : le calcul les multiplie, comme le duel le fait
            # déjà, et le rendu doit **annoncer** que la pièce est fabriquée
            # entièrement à cette qualité. Sans cette phrase, le seuil paraît
            # publié alors qu'il repose sur une hypothèse.
