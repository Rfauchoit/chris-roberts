...
def _fichier_rejets() -> pathlib.Path:
    """Les lignes illisibles, sorties de la file sans être effacées.

    Une seule ligne JSON cassée bloquait auparavant toutes les suivantes :
    `envoyer()` tentait de décoder le lot entier et recommençait indéfiniment.
    La quarantaine garde la matière de diagnostic, mais elle ne participe plus
    aux envois.
    """
    return _dossier() / "remontee-rejets.jsonl"
...
def apercu(tour: dict[str, Any]) -> str:
    """Montre exactement ce qui entrerait dans la file, après épuration.

    Ce n'est pas une promesse rédigée à côté du code : l'écran appelle la
    même fonction `usages.epurer` que l'écriture réelle. Un champ ajouté sans
    autorisation n'apparaît donc ni dans l'aperçu ni dans l'envoi.
    """
    return json.dumps(usages.epurer(tour), ensure_ascii=False, indent=2)
...
def _mettre_en_quarantaine(lignes: list[str]) -> None:
    """Conserve des lignes rejetées sans jamais bloquer la file saine."""
    if not lignes:
        return
    fichier = _fichier_rejets()
    existant = (fichier.read_text(encoding="utf-8")
                if fichier.exists() else "")
    fichier.write_text(
        existant + "\n".join(lignes[-FILE_MAX:]) + "\n", encoding="utf-8")
...
def _purger(lignes: list[str]) -> list[str]:
    """Jette l'ancien et met l'illisible en quarantaine.

    Les lignes rendues sont toutes des objets JSON : `envoyer()` peut les
    décoder une par une sans qu'un débris ancien condamne le lot entier.
    """
    ...
    for ligne in lignes:
        try:
            ...
            if not isinstance(tour, dict):
                raise ValueError("un tour doit être un objet")
            quand = time.mktime(time.strptime(
                tour["horodatage"], "%Y-%m-%dT%H:%M:%S"))
        except (TypeError, ValueError, KeyError, OSError):
            rejetees.append(ligne)
            continue
        if quand >= limite:
            gardees.append(ligne)
    _mettre_en_quarantaine(rejetees)
    return gardees
    ...
    **En cas d'échec, la file reste intacte** — c'est tout l'intérêt :
    ...
    try:
        with _VERROU:
            ...
            if not fichier.exists():
                return 0
            brutes = fichier.read_text(encoding="utf-8").splitlines()
            lignes = _purger(brutes)
            if not lignes:
                fichier.unlink(missing_ok=True)
                return 0
            if lignes != brutes:
                # Nettoyer tout de suite : si l'hôte dort, une ligne cassée
                # ne doit pas être remise en quarantaine à chaque tentative.
                fichier.write_text("\n".join(lignes) + "\n",
                                   encoding="utf-8")
        ...
        with _VERROU:
            ...
            # Des réponses peuvent avoir été ajoutées pendant l'appel HTTP.
            # On les relit avant de réécrire, sinon un envoi réussi les
            # effacerait silencieusement.
            courantes = (_purger(_fichier_file().read_text(
                encoding="utf-8").splitlines())
                if _fichier_file().exists() else [])
            ajoutees = courantes[len(lignes):] \
                if courantes[:len(lignes)] == lignes else []
            restantes.extend(ajoutees)
            if restantes:
