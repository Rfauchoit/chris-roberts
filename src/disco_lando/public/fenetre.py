...
def _poser_icone(fenetre: tk.Tk) -> None:
    """L'icône de la guilde, dans la barre des tâches et sur la fenêtre.

    Elle est embarquée par PyInstaller à la racine du bundle ; en
    développement elle vit dans `assets/`. Son absence ne doit **jamais**
    empêcher la fenêtre de s'ouvrir — une icône manquante est un détail,
    une application qui ne démarre pas ne l'est pas.
    """
    for base in (getattr(sys, "_MEIPASS", None),
                 pathlib.Path(__file__).resolve().parents[3] / "assets"):
        if not base:
            continue
        chemin = pathlib.Path(base) / "chris-roberts.ico"
        if chemin.exists():
            try:
                fenetre.iconbitmap(str(chemin))
            except tk.TclError:
                pass
            return
...
class Application(tk.Tk):
    ...
    def _page_questions(self, parent: ttk.Notebook) -> None:
        ...
        # Le partage est activé par défaut : son annonce doit donc précéder
        # la première question, pas vivre seulement dans le troisième onglet.
        # L'aperçu vient de la même épuration que la file réelle.
        annonce = tk.Frame(cadre, bg=_FOND_CLAIR)
        annonce.pack(side="top", fill="x", pady=(0, 8))
        tk.Label(
            annonce, text=remontee.ANNONCE, bg=_FOND_CLAIR, fg=_TEXTE,
            font=("Segoe UI", 9), anchor="w", justify="left",
            wraplength=820).pack(fill="x", padx=12, pady=(9, 4))
        exemple = remontee.apercu({
            "question": "où miner du quantainium ?",
            "reponse": "…", "outil": "ou_miner",
            "chemin_log": r"C:\Users\joueur\Game.log",
        })
        tk.Label(
            annonce, text=f"Aperçu pseudonymisé : {exemple}",
            bg=_FOND_CLAIR, fg=_DISCRET, font=("Consolas", 8), anchor="w",
    ...
    def _repondre(self, question: str) -> None:
        ...
        try:
            # **`ask()` rend un objet `Answer`, pas un dictionnaire.**
            # Le premier essai faisait `.get("answer")` et toute question
            # répondait « je n'arrive pas à joindre le cœur ('Answer'
            # object has no attribute 'get') » — le message accusait le
            # réseau alors que le cœur répondait parfaitement. Signalé
            # par l'utilisateur au premier lancement, 2026-08-13.
            reponse = CoreClient().ask(question)
            texte = reponse.answer or "Je n'ai pas compris la question."
            outil = reponse.tool
    ...
    def _page_raccordement(self, parent: ttk.Notebook) -> None:
        ...
        tk.Label(cadre, text="Installe l'un de ces outils et connecte-toi une "
                 ...
                 wraplength=780, justify="left").pack(fill="x", padx=16,
                                                      pady=(0, 14))
        self._etat_fournisseur = tk.StringVar(value=(
            "Choisis l'outil installé que Chris doit utiliser."))
        for etat in fournisseurs.etat():
            ...
            if not etat["installe"]:
                ...
                libelle = ("Sélectionné" if etat["selectionne"]
                           else "Utiliser ce fournisseur")
                tk.Button(
                    bloc, text=libelle,
                    command=lambda cle=etat["cle"]:
                        self._selectionner_fournisseur(cle),
                    bg=_ACCENT if not etat["selectionne"] else "#9ece6a",
                    fg="#0d1017", relief="flat",
                    font=("Segoe UI", 9, "bold"), padx=12, pady=4).pack(
                        anchor="w", padx=12, pady=(0, 10))
        tk.Label(cadre, textvariable=self._etat_fournisseur, bg=_FOND,
                 fg=_DISCRET, font=("Segoe UI", 9), anchor="w").pack(
                     fill="x", padx=16, pady=(10, 0))

    def _selectionner_fournisseur(self, cle: str) -> None:
        try:
            etat = fournisseurs.selectionner(cle)
            self._etat_fournisseur.set(
                f"{etat['fournisseur']} actif — modèle {etat['modele']}. "
                "Les prochaines questions peuvent l'utiliser.")
        except (OSError, RuntimeError, ValueError) as exc:
            self._etat_fournisseur.set(f"Raccordement impossible : {exc}")
    ...
    def _verifier_version(self) -> None:
        def chercher() -> None:
            ...
            if trouve:
                try:
                    lancee = maj.installer(trouve)
                except Exception as exc:                       # noqa: BLE001
                    message = str(exc)
                    self.after(0, lambda message=message:
                               self._version.configure(
                        text=f"Mise à jour {trouve['version']} refusée : "
                             f"{message}",
                        fg="#f7768e"))
                    return
                if lancee:
                    self.after(0, lambda: self._version.configure(
                        text=f"Version {trouve['version']} vérifiée — relève "
                             "en cours…", fg="#9ece6a"))
                    self.after(700, self.destroy)
                else:
                    self.after(0, lambda: self._version.configure(
                        text=f"Version {trouve['version']} disponible sur "
                             f"{trouve['page']}", fg="#e0af68"))
