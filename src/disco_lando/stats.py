...
STATS = {
    ...
    # Plus court vaut mieux : on recommence à tirer plus tôt.
    "reload_seconds": ("rechargement", "secondes", False),
    "item_mass": ("masse", "kg", False),
}

# D1 les ingère pour le futur moteur de duel, mais la fiche d'objet publique
# ne les rend pas encore. La dispersion n'a pas d'unité publiée et les
# paramètres de pénétration n'ont pas encore de prose fidèle : les afficher
# comme des statistiques comparables leur inventerait un sens. Cette liste
# explicite satisfait aussi la sentinelle « colonne ingérée sans couche de
# réponse » : l'exclusion est connue, mesurée et temporaire.
_STATS_DUEL_NON_PUBLIEES = frozenset({
    "spread_minimum", "spread_maximum", "spread_first_attack",
    "spread_attack", "spread_decay", "pierceability", "projectile_mass",
    "max_penetration_thickness", "projectile_radius",
    "penetration_base_distance", "penetration_near_radius",
    "penetration_far_radius", "shield_damaged", "qt_spool_time",
    "radar_aim_min", "radar_aim_max", "radar_aim_buffer",
})

# Écarte les entrées de test, ce qui n'est pas monté en vol, et **ce qui est
# soudé à un porteur**.
#
# `flightReady` + `weaponMountUsable` ne suffisent pas : le canon du porte-nefs
# Bengal les porte aussi, puisqu'il est bel et bien monté — sur un
# capital-ship. Le projet en concluait qu'« aucun champ amont ne fait la
# distinction », et comparait donc par taille faute de mieux.
#
# **C'était faux, et ça coûtait cher.** Les *tags* nomment le porteur —
# `$AEGS_Idris_Nose`, `TMBL_Storm`, `RSI_Perseus_Bespoke`, `Wolf_Gun`,
# `VanguardNose` — et 20 armes sur 142 sont dans ce cas. Sans ce filtre, le
# *Reign-3 Repeater*, arme du char terrestre Tumbril Storm, sortait 2 042 DPS
# et alpha 350 en taille 3 quand ses pairs plafonnent à 546 et 44 : le
# sélecteur de « meilleur loadout » le proposait à tout chasseur à affûts S3,
# et les duels calculés dessus étaient faux (2026-08-14).
#
# La colonne est calculée à l'ingestion, en croisant les tags avec les noms
# de vaisseaux réels — voir `ingest/loaders._porteur_exclusif`. Une base
# d'avant l'ajout n'a pas la colonne : les appelants passent alors par
# `mountable()`, qui dégrade proprement.
#: **Une arme que le jeu ne nomme pas n'est pas dans le catalogue du
#: joueur.** Cinq armes sur 140 sortent sans nom — trois canons Vanduul,
#: une tourelle automatisée Behring, un scattergun Aopoa S6 — et le
#: meilleur loadout en montait : un `VNCL_Gen2_PlasmaCannon_S2` se
#: retrouvait sur un Hawk, un Merlin et un Buccaneer, à 1 831 DPS, affiché
#: sans nom dans la réponse. Même famille que `porteur_exclusif` et que le
#: canon du Bengal : `flight_ready` et `mount_usable` ne suffisent pas à
#: dire qu'un joueur peut l'équiper.
#: **`flight_ready` et `mount_usable` décrivent le montage sur un vaisseau**,
#: et le filtre les exigeait de tout le monde. Mesuré le 2026-08-21 : ils
#: valent 0 sur les **458 armes FPS**, les 658 casques, les 468 torses, les
#: 209 sous-combinaisons, les 78 générateurs — sur tout ce qui n'est pas une
#: arme de vaisseau, en somme. Ce n'était donc pas un filtre, c'était un
#: « armes de vaisseau uniquement » déguisé : « quelles armes FPS font plus
#: de 150 DPS » répondait « je n'ai pas trouvé la donnée » alors qu'il y en
#: a **219**.
#:
#: Ce qui reste exigé de tous est ce qui vaut pour tous — pas de dev, pas de
#: porteur exclusif, un nom. Le porteur exclusif est d'ailleurs ce qui écarte
#: réellement le canon du Bengal, raison d'être du filtre à l'origine ; les
#: deux drapeaux ne servaient qu'à le doubler chez les armes de vaisseau, où
#: ils discriminent pour de bon (195 sur 202, et 186 montables).
#:
#: Vérifié dans les deux sens : les armes de vaisseau au-dessus de 500 DPS
#: restent **53**, exactement comme avant.
#: **Une attaque de créature n'est pas une arme de joueur**, et elle porte
#: parfois le nom d'une vraie. Mesuré le 2026-08-21 en ouvrant les armes FPS
#: au classement : `vlk_spewgun_ballistic_reference` — le crachat du vlk —
#: s'appelle « BR-2 Shotgun » et sort à **143 025 DPS**, devant le vrai
#: BR-2, qui en fait 132. CIG a manifestement pris le fusil comme gabarit
#: et laissé le nom d'affichage.
#:
#: Quatre entrées `vlk_` portent un nom, et **toutes les quatre** empruntent
#: celui du BR-2 (l'arme et son chargeur) ; aucun autre préfixe de faune —
#: `kopion`, `creature`, `npc_` — n'a d'entrée nommée. Le filtre est donc
#: étroit et mesuré, pas prophylactique.
#:
#: C'est le pendant de `_CONSTRUCTEURS` dans le parseur de journaux : une
#: créature s'y reconnaît aussi par son nom de classe, pas par une absence.
_FAUNE = "AND lower(i.class_name) NOT LIKE 'vlk|_%' ESCAPE '|' "

MOUNTABLE = ("AND i.is_dev = 0 AND i.porteur_exclusif IS NULL "
             "AND i.name IS NOT NULL "
             + _FAUNE +
             "AND (i.type <> 'WeaponGun' "
             "     OR (i.flight_ready = 1 AND i.mount_usable = 1))")
...
_STAT_MOTS = {
    ...
    # **Le temps de manipulation du chargeur, publié par CIG.** Le projet
    # avait conclu qu'il n'existait pas ; il est dans le fichier d'objet,
    # pas dans le résumé. Trois paliers qui discriminent chez les armes
    # FPS — 1 s, 1,5 s, 3 s — et une constante à 3 chez celles de
    # vaisseau, où la colonne ne sépare donc rien.
    "rechargement": "reload_seconds", "recharger": "reload_seconds",
}
...
#: Le vocabulaire du genre d'arme, **exposé** parce que le routeur en a
#: besoin autant que la requête. « Quels canons balistiques font plus de 300
#: dps » ne routait nulle part : `_weapon_filter` reconnaissait bien la
#: famille, mais aucun motif d'intention ne la connaissait, et
#: `objets_au_seuil` plafonnait à 2,0 — sous le seuil de confiance. Le
#: recopier dans `motifs.py` aurait fabriqué la divergence que ce projet
#: paie régulièrement ; on le lit au même endroit.
CLASSES_D_ARME = (("balistique", "ballistic"), ("ballistic", "ballistic"),
                  ("laser", "laser"), ("plasma", "plasma"),
                  ("tachyon", "tachyon"), ("distortion", "distortion"))
GENRES_D_ARME = (("gatling", "gatling"), ("repeater", "repeater"),
...
def _weapon_filter(query: str) -> tuple[str, list, str]:
    ...
    for mot, classe in CLASSES_D_ARME:
        if mot in norm:
            clauses.append("st.weapon_class = ?")
            args.append(classe)
            libelle.append(classe)
            break
    for mot, genre in GENRES_D_ARME:
        if mot in norm:
...
_ITEM_STAT_MOTS = (
    ...
    ("capacitor", "capacitor_max"), ("capaciteur", "capacitor_max"),
    ("condensateur", "capacitor_max"),
    # « Combien de temps pour recharger » est la moitié manquante de
    # « combien de balles dedans ». Aucun chevauchement avec « chargeur »
    # ci-dessus — le mot ne s'y trouve pas — ni avec la « recharge » d'un
    # bouclier, qui est un autre mot et un autre détecteur.
    ("rechargement", "reload_seconds"), ("recharger", "reload_seconds"),
...
_COMPONENT_STATS = {
    ...
    # **Le mot que le joueur emploie d'abord**, et il manquait : « quel est
    # le casque le plus **résistant** » répondait « je n'ai pas trouvé la
    # donnée » quand « les meilleurs casques » marchait déjà. La table
    # connaissait « blindage » et « protège », pas la famille de
    # « résistance » — qui est pourtant le nom que porte la statistique
    # partout ailleurs dans le projet. Faute de précision, on classe sur le
    # physique : c'est ce qu'encaisse un tir balistique, le cas ordinaire.
