...
CREATE TABLE ships (
  ...
  width             REAL,
  height            REAL,
  -- La **section réellement présentée**, de face (X) et de côté (Z), là où
  -- `length`/`width`/`height` ne donnent que la boîte englobante. L'écart
  -- est décisif pour savoir si un tir porte : l'Aurora Mk II, en H, n'occupe
  -- que 61 % de son gabarit quand l'Arrow en occupe 96 %.
  --
  -- Deux réserves mesurées le 2026-08-13, sur les 288 vaisseaux qui la
  -- publient : l'**unité n'est pas identifiable** — l'échelle au centième de
  -- m² cale la médiane à 0,43 de la boîte englobante, ce qui est crédible,
  -- mais quelques vaisseaux dépassent 1, impossible pour une section —, et
  -- `CrossSection.Y` ne correspond à aucune vue (0,05 de `L×l` en médiane).
  -- On ingère les trois, on n'exploite que X et Z, et surtout par leurs
  -- **rapports**, qui eux sont homogènes.
  cross_x           REAL,
  cross_y           REAL,
  cross_z           REAL,
  mass              REAL,
  ...
  yaw_boosted       REAL,
  roll_boosted      REAL,
  -- Le couple correcteur que l'IFCS applique réellement, de 0,1 à 1,8.
  -- « Un Aurora se pilote mal, quand tu tournes il y a énormément de
  -- latence et d'oversteer, et tu ne peux rien y faire ; l'Arrow est
  -- beaucoup plus réactif » (utilisateur, 2026-08-16). C'est cette
  -- colonne-là : Aurora 0,3, Arrow 1, et tous les Anvil à 1. La vitesse
  -- angulaire dit à quelle vitesse on tourne, celle-ci dit si l'on
  -- s'arrête où l'on visait.
  torque_imbalance  REAL,
  accel_main        REAL,
  accel_retro       REAL,
  accel_maneuver    REAL,
...
CREATE TABLE ship_combat (
  ...
  mult_energy     REAL,
  -- **Ce que l'armure encaisse elle-même**, à distinguer de ce qu'elle
  -- laisse passer. Trouvé le 2026-08-14 en cherchant comment les deux
  -- multiplicateurs publiés se composent : l'objet d'armure porte
  -- `Durability.Health` **et** `Durability.Resistance.Multiplier`, tandis
  -- que `Armor.DamageMultipliers` décrit ce qui la traverse vers la coque.
  -- Ce sont donc deux **couches** successives, pas deux facteurs sur la
  -- même étape.
  --
  -- L'écart compte : 0,81 en physique et **1,15 en énergie** en médiane sur
  -- 305 vaisseaux — au-dessus de 1, l'armure encaisse davantage l'énergie —
  -- là où ce qui la traverse vaut 0,75 et 0,60. Un laser use donc vite
  -- l'armure et fait peu à la coque ; un balistique l'inverse.
  res_physical    REAL,
  res_energy      REAL,
  shield_hp       REAL,     -- ShieldsTotal.Hp — somme des générateurs montés
  shield_regen    REAL,     -- ShieldsTotal.Regen, par seconde
  -- **Combien de temps le bouclier reste muet avant de repartir.** Publiés
  -- par générateur — `DamagedDelay` quand il a seulement encaissé,
  -- `DownedDelay` quand il est tombé — et lus sur le bouclier **équipé**,
  -- jamais sur une médiane : c'est le loadout qui décide (précision de
  -- l'utilisateur, 2026-08-14).
  --
  -- Ils varient vraiment : 32 valeurs distinctes pour le premier (médiane
  -- 5,5 s), 31 pour le second (~11 s) sur les 73 générateurs. Le modèle
  -- d'engagement se contentait d'une constante inventée de 2 s — assez
  -- courte pour que les deux camps se rechargent entre deux passes et
  -- qu'aucun duel n'aboutisse, ce qui avait fait retirer le repli tactique
  -- alors qu'il existe bel et bien en jeu.
  shield_damaged_delay REAL,
  shield_downed_delay  REAL,
  missiles_count  INTEGER,  -- Weaponry.Missiles : la théorie des missiles
  missile_damage  REAL      -- dégâts d'un missile (Damage.Total)
);

-- Données destinées au nouveau moteur de duel. Elles restent séparées de
-- `ships` et de `ship_combat` tant que le moteur historique ne les consomme
-- pas : D1 ne doit changer aucun résultat de production. Les unités inconnues
-- sont conservées brutes et qualifiées dans `duel/donnees.py`.
CREATE TABLE ship_duel_stats (
  ship_uuid          TEXT PRIMARY KEY REFERENCES ships(uuid) ON DELETE CASCADE,
  linear_accel_decay REAL,
  angular_accel_decay REAL,
  zero_to_max        REAL,
  max_to_zero        REAL,
  zero_to_boost_forward REAL,
  boost_forward_to_zero REAL,
  boost_ramp_up      REAL,
  boost_ramp_down    REAL,
  boost_idle_cost    REAL,
  boost_regen_delay  REAL,
  scm_max_drag_multiplier REAL,
  distortion_pool   REAL,
  shield_face_type  TEXT CHECK (
                      shield_face_type IS NULL
                      OR shield_face_type IN ('Bubble', 'Quadrant')),
  shield_face_count INTEGER CHECK (
                      shield_face_count IS NULL
                      OR shield_face_count IN (1, 4)),
  shield_reconfiguration_cooldown REAL
);
...
-- **L'EMP, la seule arme qui ne fait pas de dégâts et gagne des duels.**
-- Sept modèles au catalogue, portés par le Hawk, le Sabre Raven, le
-- Scorpius Antares, l'Avenger Warlock, le Vanguard Sentinel et le Mantis.
-- Ils étaient en base sans **aucune** statistique : `SCItemEMPParams` n'était
-- pas lu, alors qu'il publie tout.
--
-- Le mécanisme, mesuré (2026-08-16) : la décharge frappe d'abord le
-- bouclier, qui l'absorbe en totalité (`Absorption.Distortion = 1` sur toute
-- la flotte) en n'encaissant que 5 à 25 % (`Resistance` 0,75 à 0,95). Une
-- fois le bouclier tombé, les `distortionDamage` frappent les composants,
-- dont le pool tient entre 900 et 2 500 — donc les 2 475 du TroMag saturent
-- tout ce qui compte sur un chasseur, et un composant saturé est **éteint**
-- (`PowerRatioAtMaxDistortion = 0`). Les armes, elles, sont à 500 000 :
-- immunisées.
CREATE TABLE emp_devices (
  item_uuid         TEXT PRIMARY KEY REFERENCES items(uuid) ON DELETE CASCADE,
  charge_time       REAL,   -- secondes de charge avant décharge
  distortion_damage REAL,   -- points de distorsion délivrés
  emp_radius        REAL,   -- portée à pleine charge, en mètres
  emp_radius_min    REAL,   -- portée à charge minimale
  unleash_time      REAL,   -- durée de la décharge elle-même
  cooldown_time     REAL    -- avant de pouvoir recharger
);

-- Le pool de distorsion d'un composant : combien il encaisse avant de
-- s'éteindre, et en combien de temps il s'en remet. `DecayRate` vaut à peu
-- près `Maximum / 15` sur tout le catalogue, après un `DecayDelay` de 1,5 s
-- (taille 1) à 6 s (taille 4).
CREATE TABLE distortion_params (
  item_uuid     TEXT PRIMARY KEY REFERENCES items(uuid) ON DELETE CASCADE,
  maximum       REAL,
  decay_rate    REAL,
  decay_delay   REAL,
  -- 0 = la puissance décroît avec la distorsion accumulée ; 1 = elle ne
  -- chute qu'à saturation. Les centrales sont dans le second cas, les
  -- boucliers dans le premier.
  seuil_unique  INTEGER
);
...
CREATE TABLE ship_composants_exposes (
  ...
  nom             TEXT,
  class_name      TEXT,
  item_uuid       TEXT REFERENCES items(uuid),
  pv              REAL,
  mult_physical   REAL,
  mult_energy     REAL,
  seuil_physical  REAL,
  seuil_energy    REAL,
  distortion_maximum REAL,
  distortion_decay_rate REAL,
  distortion_decay_delay REAL,
  distortion_seuil_unique INTEGER,
...
-- Zones physiques et réseau de relais. Les colonnes normalisées servent aux
-- futurs calculs ; le JSON source est conservé à côté pour qu'une hypothèse
-- de D4 reste réfutable sans retourner au dépôt amont.
CREATE TABLE ship_zones_degats (
  id              INTEGER PRIMARY KEY,
  ship_uuid       TEXT NOT NULL REFERENCES ships(uuid) ON DELETE CASCADE,
  genre           TEXT NOT NULL CHECK (genre IN ('destruction', 'detachement')),
  nom             TEXT,
  pv              REAL,
  seuil_degats    REAL,
  brut_json       TEXT NOT NULL
);

CREATE INDEX ix_zones_degats_ship ON ship_zones_degats (ship_uuid, genre);

CREATE TABLE ship_relais (
  id              INTEGER PRIMARY KEY,
  ship_uuid       TEXT NOT NULL REFERENCES ships(uuid) ON DELETE CASCADE,
  hardpoint_name  TEXT,
  class_name      TEXT,
...
CREATE INDEX ix_relais_ship ON ship_relais (ship_uuid);

CREATE TABLE ship_relais_liens (
  id              INTEGER PRIMARY KEY,
  ship_uuid       TEXT NOT NULL REFERENCES ships(uuid) ON DELETE CASCADE,
  source          TEXT,
  cible           TEXT,
...
CREATE INDEX ix_relais_liens_ship ON ship_relais_liens (ship_uuid);

CREATE TABLE ship_topologie_brute (
  ship_uuid       TEXT PRIMARY KEY REFERENCES ships(uuid) ON DELETE CASCADE,
  destruction_json TEXT,
  detachement_json TEXT,
  relais_json     TEXT
);

-- Une valeur source suspecte n'est ni effacée ni remplacée. Elle reste dans
-- sa table et cette ligne l'empêche d'entrer silencieusement dans un profil.
CREATE TABLE anomalies_donnees_duel (
  population      TEXT NOT NULL,
  entite_id       TEXT NOT NULL,
  champ           TEXT NOT NULL,
  valeur_brute    TEXT,
  raison          TEXT NOT NULL,
  severite        TEXT NOT NULL CHECK (
                    severite IN ('bloquante', 'avertissement')),
  chemin_source   TEXT NOT NULL,
  PRIMARY KEY (population, entite_id, champ)
);
...
CREATE TABLE items (
  ...
  -- **Le rayon de boutique** — la catégorie sous laquelle CIG range
  -- l'objet en magasin, et elle est traduite (`@vehicle_class_*`).
  --
  -- Retenue par le crible du sprint 38 parce qu'elle **distingue ce que
  -- `type` confond** : mesuré, 34 couples (type, sous-type) sur 135
  -- portent plusieurs rayons. `WeaponGun/Gun` se décompose en Beam,
  -- Cannon, Gatling et Repeater — et 62 armes sur 202 n'ont pas cette
  -- famille dans leurs tags texte.
  --
  -- Comme la contenance, elle ne vit que dans les fichiers individuels
  -- de `items/`, sous le `Raw` du moteur.
  rayon             TEXT,
  -- **Le porteur auquel une arme est réservée**, ou NULL si elle se monte
  -- librement. Trouvé le 2026-08-14 : `flightReady` + `weaponMountUsable` ne
  -- distinguent pas une arme de catalogue d'une arme fixée à un seul
  -- vaisseau, mais les **tags** le disent — `$AEGS_Idris_Nose`, `TMBL_Storm`,
  -- `RSI_Perseus_Bespoke`, `Wolf_Gun`, `VanguardNose`.
  --
  -- L'incident : le *Reign-3 Repeater*, arme du char terrestre Tumbril Storm,
  -- sort 2 042 DPS et alpha 350 en taille 3 quand ses pairs plafonnent à 546
  -- et 44. Le sélecteur de « meilleur loadout » le proposait donc à tous les
  -- chasseurs à affûts S3, et les duels calculés sur cet armement étaient
  -- faux. 24 armes sur 144 sont dans ce cas.
  --
  -- Le critère se **mesure** au lieu de s'écrire : un tag qui contient le nom
  -- d'un vaisseau connu de la base. C'est ce qui garde le *Revenant Gatling*,
  -- dont le tag `ANVL_BallisticGatling_Bespoke` nomme une famille et non un
  -- appareil — et il est bien stock sur le Hornet comme sur le Buccaneer.
  porteur_exclusif  TEXT,
...
CREATE TABLE item_stats (
  ...
  -- **Le temps de manipulation du chargeur, publié par CIG.**
  -- `SCItemWeaponComponentParams.ammoRepoolParams.unstowMagDuration`, dans
  -- le fichier d'objet — absent du résumé `stdItem` que lit l'index, d'où
  -- une lecture ciblée des seuls fichiers d'arme.
  --
  -- Le projet avait conclu que « le rechargement n'existe pas en base » et
  -- s'y tenait ; l'utilisateur a insisté (2026-08-21, « il doit bien y
  -- avoir un temps de rechargement global communiqué par CIG quelque
  -- part »). Mesuré : **828 armes** le portent, sur trois paliers — 1 s
  -- pour les fusils, SMG, pistolets, fusils à pompe et snipers ; 1,5 s
  -- pour les armes d'épaule et le Scourge Railgun ; 3 s pour le lourd,
  -- Vendetta HMG en tête. Les armes de vaisseau valent 3 uniformément,
  -- donc la colonne n'y discrimine rien.
  --
  -- Ce n'est pas l'animation complète de rechargement — le nom dit
  -- « dégainer le chargeur » — mais c'est le seul temps que CIG publie, et
  -- il tranche entre un P8-AR et un Vendetta.
  reload_seconds    REAL,
  ...
  -- Dispersion du mode principal. L'unité n'est pas publiée dans la source :
  -- les nombres sont ingérés, mais D1 interdit encore leur conversion en
  -- angle ou en probabilité de toucher.
  spread_minimum    REAL,
  spread_maximum    REAL,
  spread_first_attack REAL,
  spread_attack     REAL,
  spread_decay      REAL,
  -- Balistique du projectile, distincte de la masse de l'arme elle-même.
  pierceability     REAL,
  projectile_mass  REAL,
  max_penetration_thickness REAL,
  projectile_radius REAL,
  penetration_base_distance REAL,
  penetration_near_radius REAL,
  penetration_far_radius REAL,
  dps_physical      REAL,
  dps_energy        REAL,
  dps_distortion    REAL,
  ...
  shield_damaged    REAL,       -- délai après un impact sans effondrement
  qt_jump_range     REAL,       -- JumpRange, mètres
  qt_drive_speed    REAL,       -- StandardJump.DriveSpeed
  qt_cooldown       REAL,       -- StandardJump.CooldownTime
  qt_spool_time     REAL,       -- StandardJump.SpoolUpTime, secondes
  qt_fuel_rate      REAL,
  radar_aim_min     REAL,       -- AimAssist.DistanceMinAssignment, mètres
  radar_aim_max     REAL,       -- AimAssist.DistanceMaxAssignment, mètres
  radar_aim_buffer  REAL,       -- AimAssist.OutsideRangeBufferDistance, mètres
