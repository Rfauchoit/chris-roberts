...
telemetrie_duel_app = typer.Typer(
    help="Campagnes expérimentales du futur moteur de duel",
)
app.add_typer(telemetrie_duel_app, name="duel-telemetrie")
...
@telemetrie_duel_app.command("preparer")
def duel_telemetrie_preparer_cmd(
    dossier: pathlib.Path = typer.Argument(
        ..., help="Nouveau dossier de campagne à créer."),
    build: str = typer.Option(..., "--build", help="Build exact du jeu."),
    source_commit: str = typer.Option(
        ..., "--source-commit", help="SHA complet de scunpacked-data."),
    operateur: str = typer.Option(
        ..., "--operateur", help="Handle de la personne qui capture."),
    description_capture: str = typer.Option(
        ..., "--description-capture",
        help="Logiciel, version, réglages et méthode d'extraction."),
    methode: str = typer.Option(
        "video_hud", "--methode",
        help="video_hud, export_externe ou saisie_manuelle."),
) -> None:
    """Crée le manifeste D1b et les trois gabarits CSV, sans écraser."""

    from .duel.telemetrie import (
        CampagneInvalide,
        MethodeCapture,
        initialiser_campagne,
    )

    try:
        capture = MethodeCapture(methode)
        chemins = initialiser_campagne(
            dossier,
            build=build,
            source_commit=source_commit,
            operateur=operateur,
            description_capture=description_capture,
            methode_capture=capture,
        )
    except (CampagneInvalide, ValueError) as exc:
        err.print(f"campagne refusée — {exc}")
        raise typer.Exit(1) from exc
    console.print(f"[green]campagne préparée[/] dans {dossier.resolve()}")
    for chemin in chemins:
        console.print(f"  {chemin.relative_to(dossier.resolve())}")


@telemetrie_duel_app.command("analyser")
def duel_telemetrie_analyser_cmd(
    dossier: pathlib.Path = typer.Argument(..., help="Dossier de campagne."),
    rapport: pathlib.Path | None = typer.Option(
        None, "--rapport", "-o", help="Rapport Markdown à écrire."),
    base: pathlib.Path | None = typer.Option(
        None, "--db", help="Base D1 de provenance ; défaut : base active."),
) -> None:
    """Valide les captures et produit des métriques sans inventer de loi."""
    ...
    from .telemetrie_duel import verifier_provenance_sqlite

    chemin_base = base or config.DB_PATH
    con = None
    try:
        if chemin_base.exists():
            con = db.connect(chemin_base, read_only=True)
        resultat = analyser_campagne(
            dossier,
            verifier_provenance=(
                (lambda campagne: verifier_provenance_sqlite(con, campagne))
                if con is not None else None
            ),
        )
    except CampagneInvalide as exc:
        err.print(f"campagne refusée — {exc}")
        raise typer.Exit(1) from exc
    finally:
        if con is not None:
            con.close()
    sortie = rapport or dossier / "rapport.md"
    sortie.parent.mkdir(parents=True, exist_ok=True)
    sortie.write_text(rapport_markdown(resultat), encoding="utf-8")
    valides = sum(analyse.valide for analyse in resultat.analyses)
    console.print(f"{valides}/{len(resultat.analyses)} essais valides")
    if resultat.eligible_revision:
        console.print("[green]campagne éligible à la revue sémantique[/]")
    else:
        console.print(f"[yellow]{len(resultat.blocages)} blocages — G1 fermée[/]")
    console.print(f"rapport écrit dans [bold]{sortie}[/]")
    raise typer.Exit(0 if resultat.eligible_revision else 1)


@telemetrie_duel_app.command("planifier")
def duel_telemetrie_planifier_cmd(
    dossier: pathlib.Path = typer.Argument(..., help="Campagne vide préparée."),
    vaisseau: list[str] = typer.Option(
        ..., "--vaisseau", help="Répéter 4 fois : 3 dérivation, 1 validation."),
    arme: list[str] = typer.Option(
        ..., "--arme", help="Répéter 4 fois : 3 dérivation, 1 validation."),
    distances: str = typer.Option(
        "100,300,600", "--distances", help="Trois distances en mètres."),
    base: pathlib.Path | None = typer.Option(
        None, "--db", help="Base D1 ; défaut : base active."),
) -> None:
    """Génère les 104 essais depuis les valeurs de la base D1."""

    from .duel.telemetrie import CampagneInvalide
    from .telemetrie_duel import planifier_campagne_sqlite

    chemin_base = base or config.DB_PATH
    try:
        distances_m = tuple(float(valeur.strip()) for valeur in distances.split(","))
        con = db.connect(chemin_base, read_only=True)
        try:
            total = planifier_campagne_sqlite(
                dossier,
                con,
                vaisseaux=tuple(vaisseau),
                armes=tuple(arme),
                distances_m=distances_m,
            )
        finally:
            con.close()
    except (CampagneInvalide, ValueError, sqlite3.Error) as exc:
        err.print(f"planification refusée — {exc}")
        raise typer.Exit(1) from exc
    console.print(f"[green]{total} essais planifiés[/] dans {dossier.resolve()}")
...
@app.command("inventaire-source")
def inventaire_source_cmd(
    limite: int = typer.Option(0, "--limite", help="0 = tout (5,6 Go)."),
    rapport_seul: bool = typer.Option(
        False, "--rapport", help="Ne relit rien, montre l'inventaire."),
    minimum: int = typer.Option(
        20, "--minimum", help="Seuil d'objets pour figurer au rapport."),
) -> None:
    """Lit scunpacked **en entier** et dit ce qu'aucun chargeur ne lit.

    Les trois audits précédents procédaient par échantillon — 600
    fichiers sur 21 849. Un sondage ne prouve rien sur ce qu'il n'a pas
    tiré, et c'est ainsi que la contenance de 2 073 objets a dormi
    (`docs/AUDIT_SCUNPACKED_2026-08-13.md`).

    Le balayage **reprend** où il s'est arrêté, et repart de zéro si le
    build a changé : un inventaire à cheval sur deux versions du jeu est
    pire qu'aucun.
    """
    from . import inventaire_source

    if not rapport_seul:
        mesure = inventaire_source.balayer(limite=limite,
                                           echo=console.print)
        console.print(
            f"  {mesure['lus']} fichiers lus "
            f"({mesure['deja_vus']} déjà vus) en {mesure['secondes']} s")
        console.print(f"  {mesure['chemins']} chemins distincts "
                      f"— build {mesure['build']}\n")
    inventaire_source.rapport(minimum=minimum, echo=console.print)
...
@app.command("banc-adversarial")
def banc_adversarial_cmd(
    combien: int = typer.Option(300, help="nombre de questions generees"),
) -> None:
    """Genere des questions que personne n a tapees, et mesure la couverture.

    **Le corpus du banc ne contient que ce qu on a pense a demander.** Le
    journal dit ce qui a rate ; celui-ci dit ce qui **raterait** — et la
    difference compte, parce que le joueur qui ne recoit pas de reponse ne
    revient pas le signaler.

    Trois compteurs, et pas un : un routeur trop permissif repondrait
    100 % en mentant. Les **suspectes** sont les plus precieuses — servies
    mais sous le seuil de reserve, c est la ou se cache la reponse assuree
    sur le mauvais outil.
    """
    from . import adversarial

    con = db.connect(read_only=True)
    questions = adversarial.generer(con, combien=combien)
    mesure = adversarial.bilan(adversarial.jouer(con, questions))
    exactitude = adversarial.bilan_exactitude(
        adversarial.jouer_exactitude(con))
    console.print("")
    console.print(f"  [bold]Couverture : {mesure['total']} questions[/] "
                  f"(générateur v{mesure['version']})")
    console.print(f"  servies      {mesure['servies']:4}  "
                  f"({mesure['taux_servi']:.1f} %)")
    console.print(f"  replis       {mesure['replis']:4}  "
                  f"(axes proposés, aucun outil)")
    console.print(f"  muettes      {mesure['muettes']:4}")
    console.print(f"  plantées     {mesure['plantees']:4}")
    console.print(f"  suspectes    {mesure['suspectes']:4}  "
                  f"({mesure['taux_suspect']:.1f} %)")
    console.print(f"  [dim]le joueur repart avec quelque chose : "
                  f"{mesure['taux_avec_quelque_chose']:.1f} %[/]")
    console.print(
        f"  [bold]Exactitude étiquetée[/]  {exactitude['justes']} / "
        f"{exactitude['etiquetees']}  "
        f"({exactitude['taux_exactitude']:.1f} %, "
        f"oracle v{exactitude['version_oracle']})")
    for erreur in exactitude["erreurs"][:6]:
        console.print(
            f"    « {erreur['question']} » → {erreur['obtenu'] or 'rien'} "
            f"(attendu : {', '.join(erreur['attendus'])})")
    console.print("")
    for titre, cles in (("Muettes", "exemples_muets"),
                        ("Plantees", "exemples_plantes"),
                        ("Suspectes", "exemples_suspects")):
        lignes = mesure[cles]
        if not lignes:
            continue
        console.print(f"  [bold]{titre}[/] — a lire, pas a compter :")
        for ligne in lignes[:10]:
            console.print(f"    {ligne}")
        console.print("")


@app.command("banc-structure")
def banc_structure_cmd(
    montrer: int = typer.Option(
        12, "--montrer", help="Combien d'exemples afficher par catégorie."),
) -> None:
    """Ce que l'analyse unique lit du corpus — sprint 42, US-1.

    **Elle ne route rien.** Ce rapport sert à décider si la notion de
    contrat vaut la réécriture, avant d'y toucher.
    """
    from . import banc
    from .router import structure

    con = db.connect(read_only=True)
    mesure = structure.rapport(con, banc.corpus())
    total = mesure["total"] or 1
    console.print("")
    console.print(f"  [bold]{mesure['total']} questions analysées[/]")
    for cle, libelle in (("muettes", "ni entité ni contrainte"),
                         ("avec_reste", "mots non réclamés"),
                         ("multi_contraintes", "plus d'une contrainte"),
                         ("multi_entites", "plusieurs entités"),
                         ("meme_type", "dont deux du même type"),
                         ("entites_douteuses", "entité sous 85")):
        console.print(f"  {libelle:24} {mesure[cle]:5}  "
                      f"({100.0 * mesure[cle] / total:.1f} %)")
    # **En parallèle, jamais à la place.** Les cinq contrats de l'US-2 ne
    # couvrent qu'une part du corpus par construction — cinq outils sur
    # 79. Ce chiffre dit combien de questions ils savent réclamer, pas
    # s'ils ont raison : c'est l'US-3 qui compare.
    couverts = mesure.get("contrat", 0)
    console.print(f"  [dim]réclamées par un des 5 contrats  {couverts:4}  "
                  f"({100.0 * couverts / total:.1f} %)[/]")
    console.print("")
    for titre, filtre in (
        ("Muettes", lambda a: not a.entites and not a.contraintes),
        ("Mots non réclamés", lambda a: bool(a.restant)),
        ("Entité douteuse", lambda a: any(e.score < 85 for e in a.entites)),
    ):
        exemples = [ligne for ligne in mesure["lignes"]
                    if filtre(ligne["analyse"])][:montrer]
        if not exemples:
            continue
        console.print(f"  [bold]{titre}[/] — à lire, pas à compter :")
        for ligne in exemples:
            console.print(f"    « {ligne['question'][:58]} »")
            console.print(f"      {ligne['analyse'].resume()[:120]}")
        console.print("")


@app.command("banc-contrats")
def banc_contrats_cmd() -> None:
    """Le verdict du sprint 42 — les deux routeurs, sur des chiffres.

    La règle est posée d'avance : les contrats ne remplacent le score que
    s'ils font **au moins aussi bien** sur la justesse. À égalité partout,
    on garde l'existant.
    """
    from . import banc
    from .router import contrats

    con = db.connect(read_only=True)
    bilan = contrats.comparer(con, banc.corpus())
    dedans = bilan["dans_perimetre"] or 1
    console.print("")
    console.print(f"  [bold]{bilan['etiquetees']} questions étiquetées[/], "
                  f"dont {bilan['dans_perimetre']} attendues sur les 5 outils")
    console.print(f"  contrat juste   {bilan['contrat_juste']:4} / "
                  f"{bilan['dans_perimetre']}  "
                  f"({100.0 * bilan['contrat_juste'] / dedans:.1f} %)")
    console.print(f"  routeur juste   {bilan['actuel_juste']:4} / "
                  f"{bilan['dans_perimetre']}  "
                  f"({100.0 * bilan['actuel_juste'] / dedans:.1f} %)")
    console.print(f"  contrat muet    {bilan['contrat_muet']:4}")
    console.print("")
    console.print(f"  [bold]{bilan['vols']} revendications hors périmètre[/] "
                  "— un routeur qui vole est pire qu'un qui se tait :")
    for outil, combien in sorted(bilan["par_voleur"].items(),
                                 key=lambda kv: -kv[1]):
        console.print(f"    {outil:22} {combien}")
    console.print("")
    for question, attendu, obtenu in bilan["exemples"][:6]:
        console.print(f"    « {question[:44]:46} » {attendu} → {obtenu}")
    console.print("")
...
@app.command("demarrage-windows")
def demarrage_windows(
    activer: bool = typer.Option(
        None, "--activer/--desactiver",
        help="Sans drapeau, la commande dit seulement l'état."),
):
    """Démarrer Chris à l'ouverture de session Windows, ou non.

    Le pendant de la case du lanceur, pour les fois où l'on n'a que du
    texte — une session à distance sur la machine de l'hôte, par exemple.
    C'est exactement la situation où l'on découvre qu'un cœur arrêté ne se
    rallume pas tout seul.
    """
    from .launcher import demarrage

    if activer is not None:
        if not demarrage.definir(activer):
            err.print(
                "Réglage impossible : cette machine n'a pas de registre "
                "Windows, ou Windows a refusé l'écriture."
                if activer else
                "Retrait impossible : cette machine n'a pas de registre "
                "Windows, ou Windows a refusé l'écriture.")
            raise typer.Exit(1)
    ligne = demarrage.inscrit()
    if ligne:
        console.print(f"[green]actif[/] — {ligne}")
        console.print("[dim]Se déclenche à l'ouverture de session, pas au "
                      "démarrage de la machine.[/]")
    else:
        console.print("[yellow]inactif[/] — Chris ne démarre pas tout seul.")
        prevu = demarrage.commande()
        if prevu:
            console.print(f"[dim]« --activer » inscrirait : {prevu}[/]")
        else:
            console.print("[dim]Rien à inscrire ici : ni « Chris "
                          "Roberts.exe » ni environnement Python trouvé.[/]")
...
@app.command("activites")
def cmd_activites():
    """(Ré)ingère le catalogue d'activités depuis `data/activites/`.

    Ce que le jeu ne publie pas : quoi faire un soir, avec combien de monde,
    en combien de temps. Les fiches sont versionnées dans le dépôt — une
    réingestion reconstruit `disco_lando.db` de zéro, et le socle doit donc
    pouvoir s'y reprojeter. À enchaîner après `ingest`, comme `uex`, `wiki`
    et `trad`.

    Une fiche sans source ou au vocabulaire inconnu **fait échouer la
    commande** plutôt que de s'ingérer à moitié.
    """
    from .ingest import activites

    con = db.connect(config.DB_PATH)
    try:
        if activites.sync(con, echo=console.print) == 0:
            raise typer.Exit(1)
    except activites.FicheInvalide as exc:
        err.print(f"Fiche refusée — {exc}")
        raise typer.Exit(1) from None
    finally:
        con.close()
...
def cmd_porteurs():
    """Recalcule les armes réservées sans réingérer toute la base."""
    from . import porteurs

    con = db.connect(config.DB_PATH)
    try:
        marquees = porteurs.remplir(con)
        con.commit()
    finally:
        con.close()
    console.print(f"[green]{marquees}[/] armes réservées à un porteur.")
